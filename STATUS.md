...
**最后更新**: 2026-07-02 (Sprint 190 — 运营真业务触发 × 2 bugfix + 1 endpoint). 1) SKILL.md §1.5.1 + §1.5.2 加 (关键词同义词库 + 工具缺位自检), 防 WorkBuddy 误判"daily-gsv-multi-period 工具缺位" (实际 tool 已实装 8 维度 + 多周期). 2) scripts/ad_hoc_query.py argparse adapter L4.43 bugfix 透传 nargs (Sprint 183 漏写, 导致 daily-gsv-multi-period `--periods start end start end` 多值报 unrecognized arguments). 3) backend/routers/ad_hoc_query.py 加 /daily-gsv-multi-period endpoint (跟 Sprint 188 B1 同模式: Pydantic + run_daily_gsv_multi_period 复用), 9 → 10 业务 endpoint. uvicorn launchctl kickstart -k 后 10 endpoint 实加载. SKILL.md 跟 .workbuddy/skills/ad-hoc-query/SKILL.md 软链自动同步. 3 pytest case 加 backend/tests/test_ad_hoc_query_api.py 全部 SKIPPED (Sprint 188 B1 同模式, 生产 DuckDB 锁). 累计 118 sprint 0 debt 持续 (Sprint 190 真业务触发, 跨 Sprint 60+ 0 debt stable 模式 +12 sprint), L4.x 36→**37 stable** (新增 L4.43 argparse adapter 透传永久规则), fix_pattern #74 沉淀, /document-release 累计 21 次, fix_pattern #74 跟 L4.42 SSOT 实证交叉引用 (argparse adapter 漏 spec kwargs 跟 sprint 立项漏 git grep 实证是同根问题: 实施前未跟 spec 详细对齐).
