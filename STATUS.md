...
| 项 | 状态 |
|---|---|
| VERSION / main | VERSION **0.9.0.4**（本候选）。origin/main **`fda26b64`**（#195）。loopback。**不上公网**。产品仍 PARTIAL。 |
| DSH 钉 | **0.1.6-alpha.1**（`0a15e36e`）。6677 PID **58775** 已切（原仓 runtime + workbench + shine-brand，未 `--fresh`，非官方 reload）。15173 PID **85899**。 |
...
- Git 合入不等于部署。6677 原仓 runtime PID **58775** 已 `plugin add` shine-brand + workbench。15173 PID **85899**、8000 PID **85883**、18082 PID **83173** 未动。现役未 `--fresh`。不要用会编原仓插件的官方 `reload`。工作树已收口，只留 `main`。
- `HANDOVER-CODEX.md` 不提交。下一产品：人群行动另授权、公网另授权。WATERFALL 本候选已拆未合入／未 reload。品牌已合 #194。#171 `5a8329c3`。Q3 overlay 已灌；`fill_user_rfm=0`。
