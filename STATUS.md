...
**最后更新**: 2026-07-02 (Sprint 201 R1: **Read-Write Splitting 治本并发** — 用户最终需求"看数据看板的人和取数的人可以同时进行"真因: Sprint 184 L4.38 DuckDB flock 锁模型 + L4.36 禁停 uvicorn 双锁死. 治本 6 phase: ① Read-Write Splitting (`dual_conn.py` 看板 read_only 池 + 取数 write connection, 1 write + N read_only = 0 冲突, 跟 Sprint 184 v3 文档化 1:1 stable) ② Query worker 独立进程 (`scripts/query_worker.py` + `query_worker_client.py`, 走 stdin + DuckDB 安全配置 5 项 + SQL 注入防御 + L4.47 valid_order 3 条件, 跟 Codex consult 阶段 B-F 1:1) ③ Snapshot 机制 (`dump_duckdb_snapshot.py` + launchd 5 分钟) ④ Prometheus-compatible observability (`query_metrics.py` + `/metrics` 零依赖文本输出) ⑤ W5/RFM cache read-only 降级 (避免读接口 500) ⑥ pytest 14 case 5 TestClass 锁回归. 14 files / +1077/-53 across 1 commit `8f7a933`, 累计 127 sprint 0 debt 持续 (Sprint 201 R1 1 commit 0 业务代码改动, 跨 Sprint 60+ 0 debt stable 模式 +22 sprint), VERSION 不 bump (跟 Sprint 89/167/190/191/192/193/194/195/196/197/198/199/200 0 业务代码改动 模式 stable, 累计 20 次 /document-release bump 持续), L4.x 39→**42 stable** (新增 **L4.48 进程隔离** + **L4.51 Read-Write Splitting** + **L4.52 snapshot 机制**), fix_pattern #82 沉淀 stable + **#89 (新增) — Read-Write Splitting**. pytest baseline 979/73/0 → **993/73/0** (净 +14 case: TestReadWriteSplittingBasic 2 + TestReadWriteSplittingConcurrent 2 + TestQueryWorkerIsolation 2 + TestSnapshotMechanism 2 + TestL436Compliance 2 + 4 W5 cache 配套). 全套 pytest 64 passed + ruff all checks passed + git diff --check clean. /document-release 累计 31 次真治本. main HEAD `06a7b0a` (f62a4af → 06a7b0a, 跟 Sprint 199 + Sprint 200 R1 v2.1 + Sprint 201 R1 模式 stable). Codex consult 6 补强 1:1 stable. 后续 Sprint 201 R2 14 tool 真实覆盖率 95% → 100% (3 P0, 5-6 天) + Sprint 201+ ClickHouse / Trino POC (8-10 周).
