...
**最后更新**: 2026-07-04 (Sprint 203 R2 amend: **3 P1 真 bug 治本** — Finding 2.2 dual_conn Semaphore + Finding 4.1 ClickHouse POC 启动条件监控 launchd weekly + Finding 4.6 /metrics dashboard OpsView.vue, 跟 L4.14 amend 1:1 stable. 改动: `backend/services/dual_conn.py` 加 `threading.Semaphore(READ_POOL_SIZE * 2)` 防 burst 下 DuckDB 连接无界增长 (5+ 业务分析师并发取数场景); `scripts/clickhouse_poc_monitor.py` + `scripts/launchd/com.fuqing.clickhouse-poc-monitor.weekly.plist` + `backend/tests/test_clickhouse_poc_monitor.py` 5 case 锁回归, 跟 L4.59 R6/R7/R8 1:1 stable 模式 launchd weekly 周日 04:45 跑 (跟 R6 04:00 / R7 04:15 / R8 04:30 错开); `frontend-vue3/src/views/OpsView.vue` + `/ops` route 显示 `/metrics` Prometheus 文本解析后 query 计数 + P50/P95/P99 延迟, 30s poll 跟 L4.52 observability cadence 1:1 stable. 当前 DuckDB 117.4GB < 200GB trigger → 0 触发 → PASS. 1 amend commit `9f72b23` + 1 plist 修复 amend `215c763` (L4.62 永久规则化 launchd plist XML 注释规则, plutil -lint OK 才算合规). `docs/sprints/SPRINT203_ARCHITECTURE_REVIEW.md` 375 行作为 L4.42 立项实证输入. Focused verify: `PYTHONPATH="$(pwd)" pytest backend/tests/test_clickhouse_poc_monitor.py -v` → **5 passed in 1.45s**; cross-stable `PYTHONPATH="$(pwd)" pytest backend/tests/test_clickhouse_poc_monitor.py backend/tests/test_pre_existing_fail_monitor.py -v` → **8 passed in 2.10s**. Frontend: `cd frontend-vue3 && npm run build` → **built in 1.47s**, `OpsView-CQkhtTGV.js` bundled; `npx vue-tsc --noEmit` → **exit=0**. Ruff scoped 3 files → **All checks passed**. Live verify: uvicorn PID 24996 + vite preview PID 27466 restart 后 4 端点全 200 (`/api/v1/health` + `/metrics` + `/` + `/ops`). launchd `com.fuqing.clickhouse-poc-monitor.weekly` 已 load, RunAtLoad 触发 4 次 `CLICKHOUSE_POC_MONITOR_PASS` (DuckDB 118.4GB). VERSION `0.4.14.39`. L4.x **62 stable** (新增 L4.62 launchd plist XML 注释规则, 跨 sprint plist 写法 SSOT). 累计 Sprint 60+ 0 debt stable **134 sprint** (跨 +30 sprint); /document-release 真治本累计 **39 次**. 0 业务代码改动模式: Sprint 60+ 累计 30 次 0 业务代码改动 1:1 stable (跟 Sprint 200 R1 v2.1 1:1 stable). main HEAD `215c763` (9f72b23 → `087ed7d` merge → `215c763` plist 修复 amend → push main 0 drift).)
