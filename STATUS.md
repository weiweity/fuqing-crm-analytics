...
**最后更新**: 2026-06-28 (Sprint 144 顶部筛选解耦 + TTL 派样聚合 (U先∪百补 user_id 去重, 后端聚合层, 不动 SAMPLING_CHANNELS) + YOY/MOM 同比环比 (复用 filterStore.compareMode, 0 新增状态) + 5 section 标题化 (📊总览 / 📈汇总 / 🏷️各板块情况 / 📋派样明细 / ⏱️回购周期分布) + 新增 /api/v1/sampling/repurchase-distribution 4 桶 API 15 files / +2925/-1144 (含 baseline JSON -f 强制) + 13 case (test_sampling_ttl_aggregation 4 + test_sampling_roi_yoy 5 + test_sampling_repurchase_distribution 4) + Stage 3 review 13/13 关键点 PASS + /qa-only DONE_WITH_CONCERNS (静态 4/4 PASS, UI 阻塞 on auth: uvicorn launchd detach stdout 丢失 admin password, pytest 走 isolated_duckdb fixture 跟 uvicorn 不冲突验证 OK) + /ship skill merge --no-ff main ed8ee30 + push origin main + .ship-audit.log append + L4.8 cleanup (删本地+远程 feature/sprint144-sampling-refactor) + Sprint 145 head: 3 处 code smell cleanup (compare_prefix 死分支 + Optional 前向引用冗余 + period_distribution dead field 全链路删) + /document-release 跨文档同步, pytest 803 passed / 23 skipped / 0 failed (Sprint 144 +41 passed, race flake L5.1 接受), main HEAD `ed8ee30`, 累计 67→68 sprint 0 debt, VERSION 0.4.14.157 不变 (累计 37 sprint 不 bump), L4.x 22 stable 0 新增, 跟 Sprint 141 + 141.5 Phase 1 + 142 + 143 + 144 /document-release 模式 stable)
