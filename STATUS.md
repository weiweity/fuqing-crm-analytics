...
S2-C1 代码已合 #136。V-A/B/C 文档收口后，当前接续 **S3 原型与正式壳验收**，不开启新功能、不升版本。9 月 15 日指定 Figma 矩阵已通过；正式壳 B3 九态仍 PARTIAL，见 [本轮验收记录](docs/hackathon/S3-ACCEPTANCE-2026-09-15.md)。产品接续看 [TODOS 的 M1 核心交付](docs/hackathon/TODOS.md#m1-核心交付)，不用另建 `todo.md`。交付按本仓 [ship-pr](.agents/skills/ship-pr/SKILL.md)，审查、QA、CI 与各动作授权分别核验。
...
- **下一产品缺口**不再是五类编辑／换数／回退实现。9 月 15 日正式壳已用真实 Chrome 覆盖 B3 九类状态的代表性路径和桌面／390 展示：写入前中断、落盘后丢回执、取消被拒、同幂等键重试、确认后刷新失败均有实测。控制已恢复；说明板从恢复后的 v4 经有意保存测试到 **v9**，正文完整，原板仍为 **v9**。完整分支×视口与灰色按钮逐一实点尚未穷尽，390 展开侧栏挤压正文、拒绝取消提示有歧义，仍 PARTIAL。
- S3 指定 Figma 原型矩阵已由真实 Chrome 点击通过，长内容遮挡在用户授权后局部修复并复验；示例金额仍不作 facts。完整矩阵归档在本机 `.context/checks/figma-proto-audit-20260915/REPORT.md`，正式壳本轮记录见 [S3 验收](docs/hackathon/S3-ACCEPTANCE-2026-09-15.md)。临时拦截、延迟与视口覆盖已清理，正常刷新 v9 成功；接续处理窄屏／文案偏差并补剩余矩阵。R-1（TABLE/FUNNEL 422 与越界 bash）、S4 综合 QA、S5 UAT 仍开放。G1–G6 及用户本人 U1 均不能以本次运行或 Git 文档代签。四类扩库完整变体归 M2。
