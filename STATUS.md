...
侧栏「比赛看板」入口已合 [#155](https://github.com/weiweity/fuqing-crm-analytics/pull/155)（`27052fab`）：新标签打开 `http://127.0.0.1:5173/`，DSH 不内嵌、不代理。地址写死 5173，不跟随 `FQ_FRONTEND_PORT`。编译后 DOM 已断言 `href/target/rel`。真实浏览器跳转仍受本机 5173 占用影响，不能当成比赛看板已在跑。
...
| 侧栏比赛看板入口 | [#155](https://github.com/weiweity/fuqing-crm-analytics/pull/155)，`27052fab` | 新标签打开独立 5173 前端；不内嵌、不启旧栈。真实跳转受本机端口占用影响 |
...
- Git 合入不等于部署。#155 后 6677 已 owned 切到干净 main 工作树插件（web PID 24482，`--plugin-path` 指向 `fuqing-crm-analytics-docs-155`），18082 仍 71160，未 `--fresh`。本机 5173 被无关 `electron-vite`（customer-agent-palette，PID 18305）占用，不是比赛看板；入口点击会打开该占用进程。未停无关服务。
