...
| 项 | 状态 |
|---|---|
| VERSION / main | VERSION **0.9.0.2**。origin/main **`10529251`**（#178）。loopback。**不上公网**。产品仍 PARTIAL。 |
| DSH 钉 | **0.1.6-alpha.1**（`0a15e36e`）。6677 PID **60168** 已切（原 runtime + 0.9.0.2 journal 插件，未 `--fresh`）。15173 未动。 |
...
- Git 合入不等于部署。6677 runtime 在原仓 `.context/dsh-dev/runtime`，插件 `--plugin-path` 指向 `evidence` 的 0.9.0.2 journal。比赛看板 15173 PID 36499 在 `board-main`（`10529251`）。18082 仍 73236。未 `--fresh`。不要用会编原仓插件的官方 `reload`。
