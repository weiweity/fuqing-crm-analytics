...
**最后更新**: 2026-07-02 (Sprint 187 — CI #500 / #499 / #497 累计 3 sprint CI 复发 root 因 100% 治根, `mcp_servers/fuqing_adhoc/server.py:_run_cli` Line 38 Sprint 182 L4.32 macOS 假设 (当时用 `os.environ.get("PYTHONPATH", _CWD)` 想 inherit 父进程) 被 Linux GitHub Actions runner 反噬: `actions/setup-python@v6` 默认 PYTHONPATH=. literal → 注入 env → 子 Python 找不到 backend.services → `test_subprocess_inherits_pythonpath` 100% fail 跨 Sprint 182/183/184/185/186 累计 5 sprint CI 红. 治根: `_PYTHONPATH = _CWD` 不 inherit, force str(PROJECT_ROOT) 绝对路径 (macOS / Linux 都过). pytest 893/73 skip 持续, ruff 0 errors, main HEAD `4be74bc + Sprint 187`, 累计 116→**117 sprint 0 debt** (Sprint 187 全部治本, 跨 Sprint 60+ 0 debt stable 模式 **+10 sprint**), VERSION `0.4.14.29` → `0.4.14.30` (Sprint 187 CI root 因 + L4.41 永久规则 + fix_pattern #72 + 累计 18 次 /document-release 真治本, 跟 Sprint 179/181/182/183/184/185/186 模式 stable), L4.x 34→**35 stable** (新增 L4.41 subprocess PYTHONPATH 绝对路径), 11 hook 闭环 (+ Sprint 185 L4.40 + Sprint 187 L4.41), fix_pattern #72 沉淀. 当前状态: CI #500 / #499 / #497 累计 3 sprint test job fail 测试已 PASS, push 后预期 4/4 jobs 全绿. sprint close memory 沉淀 (Sprint 187 close memory 1 个). /document-release 累计 18 次真治本
