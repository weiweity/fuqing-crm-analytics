...
- **下一产品缺口**不再是五类编辑／换数／回退实现。开放项：Figma 原型真实点击（登录墙再探仍 NOT_RUN）、B3 九态在正式壳／390、TABLE/FUNNEL 生成 422 与越界 bash（R-1 挂账不扩修）、G1–G6 勾选与 U1。390 预览→取消→确认保存链已在同一 6677 补跑（LINE 图例 false→true，原板 **v9**）。
