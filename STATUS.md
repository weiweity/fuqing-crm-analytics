...
**最后更新**: 2026-07-02 (Sprint 189 — L4.35 skill symlink 治理修 100→0 false positive). 真因: WorkBuddy 端 107 skill / Claude Code 端 81 skill, 仅 1 双端共有 (ad-hoc-query). 其他 106 是 WorkBuddy 生态独占 (brainstorming/pdf/xlsx/amazon 等), 跟 L4.35 SSOT 无关 — 之前 100 false positive drift warning 是 session_start_check 设计假设错误. 修法: scripts/session_start_check.py:_verify_skill_symlinks Sprint 189 升级 (line 92-131) 加跳过逻辑, 仅双端都有 SKILL.md 的 skill 才校验. 真跑: `1 OK / 0 drift (skip 100 workbuddy-only / claude-only)`. 0 业务代码改动, 跟 Sprint 89 / 167 0 commit 模式 stable. pytest 不退化, ruff 0 errors. 累计 118 sprint 0 debt 持续 (Sprint 189 纯治理, 0 业务代码改动), L4.x 36 stable 0 追加 (L4.35 永久规则升级), /document-release 累计 20 次.
