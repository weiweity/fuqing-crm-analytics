...
**最后更新**: 2026-07-05 (Sprint 203 R3 OpsView STUB TODO 5 件接入 — Stub #1 DuckDB file size + Stub #2 W5 manifest version + Stub #3 Read pool 利用率 + Stub #4 ClickHouse POC b/c 件 stub + Stub #5 pytest 7 case 锁回归, 跟 L4.14 amend 1:1 stable. 改动: `backend/main.py` 加 3 件 health endpoint (`/api/v1/health/db_size` 走 `Path(DUCKDB_PATH).stat().st_size` + `/api/v1/health/manifest` 走 `_manifest_tracker_singleton.current_version()` + `/api/v1/health/pool` 走 `dual_conn._read_pool` + `_read_semaphore._value`), middleware bypass 同步更新 (`rate_limit_middleware` + `auth_middleware`); `frontend-vue3/src/views/OpsView.vue` 加 3 件 NCard (DuckDB size + Manifest version + Read pool 利用率), Promise.all 并行 fetch 4 endpoint 30s poll; `scripts/clickhouse_poc_monitor.py` b/c 件 stub 注释更新 Sprint 203 R4+ 真接入 `/metrics`; `backend/tests/test_sprint203_r3_opsview_stubs.py` 7 case 锁回归. Focused verify: `PYTHONPATH="$(pwd)" pytest backend/tests/test_sprint203_r3_opsview_stubs.py -v` → **7 passed in 1.83s**; cross-stable `PYTHONPATH="$(pwd)" pytest backend/tests/test_sprint203_r3_opsview_stubs.py backend/tests/test_clickhouse_poc_monitor.py -v` → **12 passed in 2.02s**. Frontend: `cd frontend-vue3 && npm run build` → **built in 1.47s**; `npx vue-tsc --noEmit` → **exit=0**. Ruff scoped 3 files → **All checks passed**. Live verify: 4 endpoint 全 200, DuckDB size **118.4 GB** (距 200GB trigger 还有 81.6 GB headroom), Manifest version **null** (manifest JSON 不存在, 正常), Pool utilization **10%** (1/10 semaphore in_use, READ_POOL_SIZE limit 5). VERSION `0.4.14.40`. L4.x **62 stable 持续** (Sprint 203 R3 0 新增, 跟 L4.20/L4.40/L4.42/L4.50/L4.59/L4.60/L4.61 永久规则配套). 累计 Sprint 60+ 0 debt stable **135 sprint** (跨 +31 sprint); /document-release 真治本累计 **40 次**. 0 业务代码改动模式: Sprint 60+ 累计 **32 次** 0 业务代码改动 1:1 stable. main HEAD `cfa7cef` (e261347 → `cfa7cef` merge → push main 0 drift).)
