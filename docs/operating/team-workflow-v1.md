...
| **e2e** | ✅ **挡 merge**（2026-07-19 根治：TEST_MODE + seed DB；与 lint/test 同级） |
...
| CI | lint + test 全量（+ deselect SSOT）+ e2e（TEST_MODE + seed） |
| 生产 | pull；按需重启 |
...
| e2e 挡 merge | ✅ 2026-07-19 根治后去掉 continue-on-error；`FQ_CRM_TEST_MODE=1` + seed |
...
| 预发环境 | 📋 未做 |
| CLAUDE L4 表下沉 | 📋 未做 |
| e2e 修稳 | ✅ 2026-07-19 根治（#e2e-data 闭环） |
