...
当前 4325 运行 `6202e89`，CI SUCCESS；18083 API 未重启，模型配置和原板保持。真实等价问法的数值、条件与结果/运行标识通过，完整 T13 及金额单位/合同归属等表述问题仍开放，详见[当前候选与评测](PRODUCT-UAT-CANDIDATE-2026-09-10.md)。

[单位来源修复](COMPUTED-UNITS-DELIVERY-2026-09-10.md)已完成本地合同、持久化与编译后 DOM 验证；当前合成源单位保持 UNKNOWN。代码尚未切换到运行实例，真实模型复测仍待执行。
