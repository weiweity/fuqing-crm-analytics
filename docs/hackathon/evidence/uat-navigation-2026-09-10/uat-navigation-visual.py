...
for width,height in [(1440,1000),(1024,900),(390,844)]:
 ...
 b.run('reload');b.run('click','[data-testid="analytics-b0-open"]')
 ...
 for attempt in range(4):
  opened=b.js('({expanded:document.querySelector(".sm-board-picker input").getAttribute("aria-expanded")})')
  if opened['expanded']=='true':break
  b.run('press','ArrowDown');time.sleep(.15)
 else:raise RuntimeError('keyboard did not open picker')
 ...
 state['keyboard']=[]
 for attempt in range(4):
  active=b.js('({text:document.getElementById(document.querySelector(".sm-board-picker input").getAttribute("aria-activedescendant"))?.innerText,expanded:document.querySelector(".sm-board-picker input").getAttribute("aria-expanded")})')
  state['keyboard'].append(active)
  if active['expanded']=='true' and '42e5261f' in (active.get('text') or ''):break
  b.run('press','ArrowDown');time.sleep(.15)
 else:raise RuntimeError('keyboard did not reach retail option')
 b.run('press','Enter');time.sleep(.15)
