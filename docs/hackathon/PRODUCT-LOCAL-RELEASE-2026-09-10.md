...
4325/18083 均为 `84b0ac8`，CI 34441966415 SUCCESS；当前 PID 分别为 72500、72472。五库恢复后已切换并通过局部真实复测，现有 10 条结果、2 块板、原草稿 v2。新旧插件、旧代码、当前备份与运行绑定见[单位候选记录](COMPUTED-UNITS-CANDIDATE-2026-09-10.md)；完整 T13 仍 PARTIAL。

## 备份与恢复证据

**单位 v2 恢复已实测**：units-release-47ndx_76/state-backup 是五库原备份，包含 2 块板、8 条结果与原草稿 v2。独立新旧恢复目录各追加过一条测试结果，不作为原备份使用。旧读者读取新 v2 返回 409。当前活状态已有两条新 v2，回退保留活状态，旧代码只使用原备份恢复出的新目录。以下四库内容仅为早期历史记录。
...
4. 当前五库备份已验证新旧读者读取、各自写入和新进程重读；旧读者读取 v2 返回 409。旧版完整浏览器降级未验证，不能宣称无损降级。当前备份保留原草稿 v2，回退会暂时看不到本次两条新结果，升级后的目录必须保留。
