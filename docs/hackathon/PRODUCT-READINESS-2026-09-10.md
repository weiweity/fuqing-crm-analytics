...
> 当前增量：2026-09-16 本候选 G1–G6／U1 已在 [TODOS](TODOS.md#m1-核心交付) 记账；G3 现场确认保存新板 v8→v9，现役 6677 PID **14287**。说明板仍 v15、原板仍 v9。[S4](S4-QA-2026-09-15.md) 隔离链仍是 9 月 15 日基线。[S5](S5-UAT-HANDOVER-2026-09-15.md) 指向 6677／15173。完整 G5 视觉／T16／触控仍开放。非正式 release。

> 上一刀：[R-1 生成配置错误恢复](R1-GENERATION-RECOVERY-2026-09-15.md)已合 [#153](https://github.com/weiweity/fuqing-crm-analytics/pull/153)（`698278e`）。用户已授权实施的 [S3 界面修复](S3-UI-REPAIR-2026-09-15.md)完成窄屏覆盖导航、取消拒绝与中文连接错误。85 历史跳转未定因。下面接续段保留修复前记录；整体仍 PARTIAL。

> S3 交叉补验时点（不是当前任务卡）：[S3 续验](S3-ACCEPTANCE-2026-09-15.md)指定 Figma 矩阵通过；[交叉补验](S3-CROSS-MATRIX-2026-09-15.md)补齐正式壳核心恢复分支双视口及12次忙态恢复按钮实点。85 桌面取消忙态意外返回对话仍未定位；88 最小复测阻断未生效，实际新增同正文 v15，记 SETUP_FAILED。窄屏侧栏挤压与取消/连接文案已由 [S3 界面修复](S3-UI-REPAIR-2026-09-15.md)处理，不再当作开放缺陷。当前 Git／入口／G 门槛看 [STATUS](../../STATUS.md)、[S4](S4-QA-2026-09-15.md)、[S5](S5-UAT-HANDOVER-2026-09-15.md) 与 [TODOS](TODOS.md#m1-核心交付)。整体 PARTIAL；未正式 release。本页下文是 9 月 10 日历史记录，端口、工作树及授权按历史时点理解，不自动重启服务或沿用旧调用预算。
...
`84a4eb3` 的 CI `34426683769` 已核验 SUCCESS；用户候选仍为 4325 上的 `9948de3`，模型配置与 18083 API 未改。新的隔离夹具通过系统目录选择及取消、Standard/Minimal/PTC/Creator/自定义 preset 实际文件执行、插件 off/on 组合保留和原会话恢复。原生 `/compact` 压缩 12 个历史项约 10,613 tokens，摘要落盘，重启后后续模型请求包含摘要且文件读取成功。

[完整补验记录](PRODUCT-NATIVE-EXTENDED-2026-09-10.md)保留夹具失败及修正、原生日志和进程退出证据。33 次请求均在本地 mock 服务，含一次标题路由失败；不计入 T13。手动压缩不代表自动溢出或真实模型摘要质量通过；完整 DESIGN、业务边界、本人 T15、正式 T16 及发布条件保持开放。本段更新上文对应历史时点的“目录/preset/手动压缩未验”状态，不扩大其他通过结论。
