...
| S2 真实 AI 核心链 | 六类自由组板；METRIC 标题修改取消／确认 v2／刷新重开；生成快捷入口 | S2-C1、其余五类真实模型编辑、LINE 日期换数与当前模型板回退 |
| S3 设计与实现对齐 | 六类合同、代表性修改及保存恢复设计；隔离原生组合／布局分项 | 正式 Figma 点击、属性与真实数据投影、正式壳下完整状态及窄屏对应 |
...
9 月 13 日真实模型证据：S2-A 为 3 prompt／9 step／7 工具，GSV 仅问数一次；S2-R2受控批次为 5 prompt／11 step／6 工具，0新增问数。另一次人工中止、无完整回执的轮次不计通过。两次 remote 注入失败保留为历史，现已修复；不是六类编辑全部通过。本轮 Git 收尾没有新增模型调用或重载。

### 下一刀：S2-C1 方案 B

- [x] 只读定位：UI／SQLite原板为 APPLIED/v2，未覆盖；确认动作未进入原生会话，下一轮 catalog 不含已存板状态。模型把历史 PREVIEW_READY 当当前状态，不是存储丢失。
- [ ] 在目录上下文增加**同权限、同会话**的 `saved_boards` 摘要（id/title/version）；标明分页／未知，不返回整份 facts，不跨会话泄露。
- [ ] 方法包与生成快捷入口明确“已保存不是草稿；GENERATE 是另一份未保存新板”。不自动 queue/steer，不加模型 confirm 工具，不改 GENERATE 新 board 的语义。
- [ ] 隔离红→绿验证已存 v2 可见、权限／会话隔离、未知不得声称已保存；拒收不得回滚。模型具体措辞需后续有界真实复验，单元测试不代签。

方案 B 方向已获确认，但实现尚未做；Git 收尾完成后再开这张任务卡。原定位正文位于本机 `.context/checks/ai-cockpit-goal/s2-c1-investigation-m3vN9c/RESULT.md`，不作为公共可访问证据。

### S2 剩余真实模型用例

每批先登记模型调用预算、超时／停止条件及维护窗口，从已有失败点续测；保留原模型设置，不用手写工具调用代替模型自主选择。每例覆盖预览→取消不变→重提→确认→重开，其他块／属性／布局／facts／来源不变。

| 类型 | 待验目标 |
|---|---|
| LINE | 以初始值相反的图例开关验证；另改至合成覆盖内8月1–15日，取得新结果且仅换目标来源，原31日结果及邻块保留 |
| BAR | 数值标签开关，不改变绑定值或重查数 |
| TABLE | 仅显示实际 `period` 列；当前结果只有 period/gsv，不编造 Figma 示例占比列 |
| TEXT | 修改说明正文，不冒充已核验事实 |
| EVIDENCE | 修改摘要，保留原来源／结果 ID／证据 |

METRIC标题路径已验，不重复当作全新成功；样式操作核实际问数工具回执增量0，日期变更核新 request/result，不能只比较目录条数。取消新数据提案不撤销已经执行的查询；结果未知不能视为取消成功。
