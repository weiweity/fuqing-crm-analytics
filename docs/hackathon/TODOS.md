...
2026-09-15 当前增量：[R-1 生成配置错误恢复](R1-GENERATION-RECOVERY-2026-09-15.md)已本地修复并通过隔离原生工具／HTTP 回归，B0 完整检查与干净重建通过。历史“FUNNEL 422”定位为 LINE/TABLE 重叠，修正布局可保留 FUNNEL；bash 历史尝试已被拒绝。真实模型不再越界尝试的行为尚未复验，本批未提交，不据此关闭 S2/M1。
