# 旧 CRM L4 技术细则与历史记录（按需）

> **当前适用边界（2026-09-06）**：行为规则统一见 [AGENTS.md](../../AGENTS.md)。本文件保留旧 CRM 技术依据与失败历史，只按本次触及主题读取，不是新工作台任务清单或启动许可。历史自动 push/删分支、安装 launchd、禁止停服务、指定旧模型、无条件全量/多轮审查等流程均不再作为当前指令；有效 SQL、字段及平台契约仍须按实际调用链核验。

> 以下是 L4.x 历史全文，不维护第二份 Agent 行为规则。
