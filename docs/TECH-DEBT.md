...
**最后更新**: 2026-06-25 (Sprint 116 真 refactor 修 #D7-#D10: 抽 _prune_lib 解耦 cleanup_backups ↔ backup_duckdb (留尾治理 sprint, 修 Sprint 112 /review defer #D7-#D10, 跟 Sprint 99+100+101+102+103+104+105+110+111+112 模式 一致), 5 文件 +498/-110 行, pytest 27 passed / 23 skipped / 0 failed (+3 vs Sprint 112 24 baseline), 累计 59 sprint 0 debt 持续 (+1 vs Sprint 112), L4.x 22 stable 0 新增, VERSION 0.4.14.157 不变, /review 4 项 defer 加 #D11-#D14 留尾 Sprint 117+ 一起修 (L4.21 反 sprint 自我反馈闭环遵守))
**当前债数**: 0 条 (Sprint 112 持续, Sprint 116 /review 4 项 defer 标 #D11-#D14 留尾 Sprint 117+ 一起修, 当前债数仍 0 因为 defer 不算当前债)
**跨 sprint 留尾**: 4 条 (Sprint 116 新增 #D11 _prune_lib '_' 前缀 PEP 8 + #D12 _matches_magic log observability + #D13 case-sensitive glob + #D14 longest-wins dict order, 留尾 Sprint 117+ 真 refactor sprint 一起修; Sprint 112 #D7-#D10 Sprint 116 已闭环; D1 50m-scale 为按 30M 数据量触发的推后决策, 不计当前债)
...
- 📋 **#D11 _prune_lib '_' 前缀违反 PEP 8 private 约定 (Sprint 116 /review maintainability INFORMATIONAL defer)**: scripts/etl/common/_prune_lib.py 用 '_' 前缀 (Python private 约定), 但 cleanup_backups.py + backup_duckdb.py + test_sprint116_lsof_missing_path.py 都从外部 import _prune_lib._prune_with_safety. 跨模块访问 private 符号违反 PEP 8. Sprint 116 解耦成功但命名不规范. Sprint 117+ 修 (rename scripts/etl/common/_prune_lib.py → scripts/etl/common/prune_lib.py, 跟 scripts/etl/common/lark.py 命名一致)
- 📋 **#D12 _matches_magic log observability regression (Sprint 116 /review maintainability INFORMATIONAL defer)**: _matches_magic 返回 False 时 _prune_with_safety log 'prune: skip {p.name} (magic check failed)' (line 135) 丢掉 offset + actual magic bytes 信息, 跟 Sprint 60+ 留尾 #D7 修法初心 (debug 误 glob 错) 冲突. Sprint 116 改 _matches_magic 应回传 reason, caller log 完整信息. Sprint 117+ 修 (_matches_magic 改返 tuple[bool, str], _prune_with_safety 用 reason log 'expected PAR1@0, got XXXX@0' 好诊断)
- 📋 **#D13 case-sensitive glob mismatch (Sprint 116 /review maintainability INFORMATIONAL defer)**: _matches_magic 用 `str(p).endswith(suffix)` 字符串匹配, 大小写敏感. cleanup 实际 glob 在 macOS APFS case-preserving 但 Linux HFS+ default case-insensitive. .parquet / .duckdb 大小写不一致的文件会被 glob 命中但 magic check 跳过. Sprint 117+ 修 (`str(p).lower().endswith(suffix.lower())` 或 `PurePath(p).suffix` lowercase 化 suffix)
- 📋 **#D14 longest-wins 依赖 dict iteration order (Sprint 116 /review maintainability INFORMATIONAL defer)**: _matches_magic 用 'len(suffix) > len(matched_suffix)' 选最长后缀, 依赖 dict iteration order (Python 3.7+ insertion order) — MAGIC_CHECKS 顺序 .parquet / .duckdb / .duckdb.zst 保证 .duckdb.zst 最后 (最长), 后人加新 suffix 不注意顺序会引入 bug. Sprint 117+ 修 (`sorted(MAGIC_CHECKS, key=len, reverse=True)` 显式 sort 或加注释 'insertion order matters: longest first')
