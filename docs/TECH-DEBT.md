...
**最后更新**: 2026-07-03 (Sprint 201 R2 v24 + 201+ v5: **L4.42 立项实证 + 7 case test SSOT 对齐** — 你 7/3 立项 spec 描述 5 任务 (任务 A/B/C 3 P0 业务补全 + 任务 D 4 case 修复 + D-5 w4_t7 4 case 闭环). Codex Stage 2 L4.42 实证验证: 任务 A/B/C 0 业务触发 (git log + grep 0 hit 业务方真邮件/工单, 跟 Sprint 199 R1 + Sprint 188 B3 反漂移 1:1 stable) → 0 commit 收口 (留尾登记在此 TECH-DEBT); 任务 D 5 case 真在 FAIL (D-1 PercentageField Pydantic v2 str() 不含 alias + D-2 MOM Sprint 145 改 compare_prefix 后 stub data 反推 -9.09% 而非 100% + D-3/D-4 period_distribution 字段 Sprint 145 删后 test 没改) → 0 业务代码改动修复. 4 files / +375/-71 across 1 commit `79e5d33`, 累计 128 sprint 0 debt 持续 (跨 Sprint 60+ 0 debt stable 模式 +24 sprint), VERSION **不 bump** (跟 Sprint 89/167/190-202 0 业务代码改动模式 stable, 累计 21 次 /document-release bump 持续), L4.x 42→**42 stable** (新增 **L4.55 (立项前 L4.42 实证 SOP 永久规则化)**). pytest baseline 993/73/0 → **1057/73/3** (净 +64 case: Sprint 202 R1 7 case + Sprint 201 R2 v23 L2 5 case + Sprint 201 R2 v24 7 case 删 5 + 修 2 + 累计 baseline 上升; 3 pre-existing failed 跟本次改动 0 关联, git stash 实证同 3 fail 跨 sprint stable). ruff scoped check All checks passed. 当前债数仍 0, 跨 sprint 留尾仍 0, 已修复累计 60+ 条. 后续 Sprint 201+ ClickHouse / Trino POC (8-10 周, 1-2 人月, 替代 DuckDB 单文件 117GB 治本业务方反映慢) + 任务 A/B/C 真业务触发再立 (P3 ad-hoc-query 工具补全, 留尾登记))

**留尾** (0 commit, 0 业务代码改动, docs/TECH-DEBT.md 登记):
- **#S201R2-v24-A**: 任务 A 淘客渠道每月明细 — extend `daily_gsv_multi_period` + `months_axis`. L4.42 实证 0 业务触发 (git log + grep 0 hit 业务方真邮件/工单). 跟 Sprint 199 R1 1:1 stable, 留尾等真业务触发再立. P3 (用户问才做)
- **#S201R2-v24-B**: 任务 B 单品按月按 spu_product_class — extend `fixed-product-list-compare-http` + `granularity_axis`. L4.42 实证 `spu_product_class` 已是 backend 真实字段 (database.py:62 VARCHAR), 不需要新增. P3 (用户问才做)
