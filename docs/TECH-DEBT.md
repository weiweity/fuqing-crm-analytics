...
- **#S202+-1-ClickHouse-POC**: ClickHouse / Trino POC 启动条件监控. L4.42 实证 0 触发: DuckDB 117GB (实证 `du -sh`), 0 业务方反映查询慢 (git log 0 hit), 0 业务分析师并发量化数据. 续期触发: (a) DuckDB > 200GB 或 (b) 查询 P95 > 30s 持续 1 周 或 (c) 5+ 业务分析师并发取数. 跟 Sprint 201+ ClickHouse POC 1:1 stable 续期. P3 (真业务触发再立)
- **#S202+-2-ETL-wall_min**: Sprint 202 R1 跑批 wall_min 业务验证. L4.54 优化 1+2 已落地 (commit `7201e84`), 期望 46min→<15min. 7/3 跑的 46min 是 L4.54 落地前 baseline. 续期触发: 业务下次跑 ETL 自动验证 wall_min < 15min. P3 (跨 sprint 自然触发)
- **#S202+-3-3P0-业务补全**: Sprint 199+ 3 P0 业务补全 (任务 A 淘客渠道每月明细 + 任务 B 单品按月按 spu_product_class + 任务 C 8 分组 TTL 扩 CATEGORY_GROUPS 4→8). L4.42 实证 0 业务触发 (git log + grep 0 hit 业务方真邮件/工单). CATEGORY_GROUPS 当前是 3 大类 dict (妆品/械品/淘客品, fixed_product_list_compare.py:36), "扩 4→8" 是凭印象 (L4.42 反漂移根治). 跟 Sprint 201 R2 v24 + Sprint 199 R1 + Sprint 188 B3 1:1 stable 续期. P3 (用户问才做)
