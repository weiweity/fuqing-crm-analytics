...
**最后更新**: 2026-07-01 (Sprint 185 — CI 跨 3 sprint 复发 100% 治根 + L4.39 macOS-only test skipif + L4.40 post-merge 自动 branch_cleanup + TestSprint183L4CrossPlatform 跨平台 class 拆分 + Sprint 184 self-zombie pre-push hook 卡点治根. pytest 893/73 skip 持续 (本地), CI Linux 期望 sprint 185+ 后 4/4 jobs 全绿, main HEAD `00fbdfe + Sprint 185 squash`, 累计 114→**115 sprint 0 debt** (Sprint 185 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +8 sprint), VERSION 0.4.14.27→**0.4.14.28** (Sprint 185 CI 复发治本 + L4.39/40 永久规则 + 累计 16 次 /document-release 真治本, 跟 Sprint 179/181/182/183/184 模式 stable, 累计 7 次 /document-release bump 持续), L4.x 32→**34 stable** (新增 L4.39 macOS-only test 必须 skipif + L4.40 post-merge hook 自动 branch_cleanup, Sprint 184 沉淀 fix_pattern #70, Sprint 185 沉淀 fix_pattern #71 累计 4 模式), 11 hook 闭环 (+ post-merge 自动 branch_cleanup 配套 L4.31, 累计 12 hooks). 当前债数仍 0, 跨 sprint 留尾仍 0, 已修复累计 60+ 条 (Sprint 185 没新增任何债, 仅 Sprint 181 治本遗留 25 个 test 都有 `os.chdir(tmp)` 风险等真业务触发)
