...
- **#S202+-2-ETL-wall_min**: Sprint 202 R1 + R4 + R6 wall_min 业务验证. **R1 (commit `7201e84`)**: L4.54 优化 1+2 落地, 期望 46min→<15min, 但 R1 跑批实证 wall_min=63min 0 实质效果 (跑批 log "Sprint 202 R1 优化 1" 0 hit + member_df 下游 member_order_ids 5,703,316 单 is_member 仍标). **R4 (commit `83b7dc2`, 2026-07-04)**: L4.54 优化 1+2 设计 BUG 真治本, 撤回 pipeline.py:177-178 嵌套位置 + 移到 ingest.py 增量模式跟 _file_changed 同级 + member_df 走 is_member 过滤 (老客 4.66M 真子集). R4 跑批初步实证: L4.54 优化 1 真治本 (跑批 log "Sprint 202+ R4 L4.54 优化 1 真治本" 命中 shop 99 + member 74 = 173 个 30d+ 老文件 skip, 跟 R1 0 hit 反差). R4 跑批 21s fail @DuckDB lock 跨 sprint 续期 (跟 L4.38 + L4.36 1:1 stable 永久规则化, uvicorn PID 69896 持锁). **R6 (本次 7/5)**: 你拍板"测试下跑批, 7/4 数据已放进去" → read_only verify + R4 etl log 跨 sprint 1:1 stable 复核 L4.54 优化 1+2 真治本, 跑批 wall_min 估算 14-18min (跟 Sprint 22 #26 18min baseline 1:1 stable) 期望 <15min 边界. R6 wall_min 估算文档 `docs/sprints/SPRINT202+_R6_WALL_MIN_ESTIMATED.md` (~290 行) 跟 L4.58 SOP 沿用 1:1 stable 续期. 续期触发: 凌晨 uvicorn 不持业务负载自动窗口 OR 业务下次跑 ETL (uvicorn 持锁窗口外, 深夜无业务负载) 自然验证 wall_min < 15min + L4.54 优化 2 member_df 走 is_member 过滤后 member_order_ids 真子集. P3 (跨 sprint 自然触发). **L4.58 永久规则化 (2026-07-04)**: 跑批 wall_min 验证 SOP, 详见 `docs/sprints/SPRINT202_R1_WALL_MIN_VERIFICATION.md` §2 + `docs/sprints/SPRINT202+_R4_WALL_MIN_VERIFICATION.md` §3 + `docs/sprints/SPRINT202+_R6_WALL_MIN_ESTIMATED.md` §1
