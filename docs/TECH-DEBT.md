...
- 📋 **#D7 cleanup_backups.py .parquet + .duckdb 不走 ZSTD magic check (Sprint 112 /review testing specialist CRITICAL defer)**: Sprint 112 抽 _prune_with_safety 后 cleanup_backups.py 的 PATTERNS = ('*.parquet', '*.duckdb', '*.duckdb.zst') 调 _prune_with_safety 时 zstd magic check 仅对 .duckdb.zst 文件生效 (line `if str(p).endswith(ZST_SUFFIX):`), .parquet + .duckdb 文件跳过 magic check 直接进 lsof + delete. Sprint 62.5 旧 _prune_old_backups 只 glob *.duckdb.zst 没这问题, Sprint 112 refactor 后暴露真治本 refactor gap. Sprint 113+ 修 (加 PAR1 magic check for *.parquet + DUCK magic for *.duckdb, 或 caller-side caller 负责)
- 📋 **#D8 cleanup_backups.py 拉起 backup_duckdb 模块 = 拉起 lark SDK (Sprint 112 /review maintainability specialist INFORMATIONAL defer)**: cleanup_backups.py module-load 时 `from scripts.etl import backup_duckdb` 无条件 import, backup_duckdb.py 顶部有 `from scripts.etl.common.lark import _send_lark_alert` + lark SDK 加载 (即使 _prune_with_safety 路径不调 lark). cleanup_backups.py 跑 launchd daily 凌晨 3 点, 拉起 lark SDK + webhook config 是隐性 side effect. Sprint 113+ 修 (抽 _prune_lib.py 解耦, 或 TYPE_CHECKING 隔开 lark import). 跟 Sprint 62 P3 launchd sandbox 教训同根因
- 📋 **#D9 deleted_names observability regression (Sprint 112 /review maintainability specialist INFORMATIONAL defer)**: Sprint 111 main() 有 `deleted_names: list[str]` 收集 + summary `| files: ...` 字段 (line 96), Sprint 112 简化后丢失 (._prune_with_safety 返 int 不返 names). `/tmp/fuqing-backup-cleanup.log` 历史日志 Sprint 111 跑出来的 `| files: a.duckdb.zst b.duckdb.zst` 行, 外部日志解析器 (lark 告警 / disk-usage-monitor cron) 都依赖这字段. Sprint 113+ 修 (_prune_with_safety 返回 Tuple[int, list[str]], caller 拿 names 拼回 summary 字段). 跟 Sprint 99 SSOT 漂移反思同根因 — 接口改造前必 verify 现有消费者
- 📋 **#D10 lsof FileNotFoundError 路径 coverage (Sprint 112 /review testing specialist INFORMATIONAL defer)**: _prune_with_safety line `lsof` subprocess.run 无 timeout, FileNotFoundError 走 'pass 保守放行' 跳过 lsof check. 真生产 macOS launchd 有 lsof, 但 CI Linux runner 没装 lsof → 实际 5 safety check 而不是 8. Sprint 113+ 修 (加 case 验证 _prune_with_safety 在 lsof 不可用时 (FileNotFoundError) 仍能删 candidate 文件, 验证保守放行行为, 跟 Sprint 95+96+96.5 e2e CI runner 教训一致)
...
| #S105-1 Sprint 105 follow-up #1: CRITICAL #3 SIGTERM fallback 死循环 | P2 | 📋 推后 (Sprint 106+) | 1% 实战触发, 改 pkill + bootout 重试 7 行, 跑 run-etl.sh line 113-119 |
| #S105-2 Sprint 105 follow-up #2: CRITICAL #4 cross-user launchctl | P2 | 📋 推后 (Sprint 106+) | cross-user check 早退 3 行, 0 实战触发, 跟 L4.6 DUCKDB_PATH 同根 hidden assumption 教训 |
| #S105-3 Sprint 105 follow-up #3: HIGH #3 DuckDB 锁 holder PID 白名单 | P3 | 📋 推后 (Sprint 106+) | 留尾, 0 实战触发 (ETL 还没起, $$ 跟子进程不在 lsof 内), 跑 run-etl.sh line 129-153 |
| #S105-4 Sprint 105 follow-up #4: HIGH #6 HEALTH_API_KEY 不一致 (留尾 #12 旧) | P3 | 📋 推后 (Sprint 106+) | 跨 sprint 留尾, Sprint 60+ 已记但没治本, launchd 起的 uvicorn 读 plist EnvironmentVariables 而非 shell var, 跑 run-etl.sh line 204 |
| #S105-5 Sprint 105 follow-up #5: MEDIUM (6) + INFO (4) 留尾 | P3 | 📋 推后 (Sprint 106+) | /review skill 6 MEDIUM + 4 INFO findings, log 治理 / set -e + pipefail 已必修 / ad-hoc platform 检查 模式稳定 |
