...
**最后更新**: 2026-06-25 (Sprint 120 commit-msg drift hook 调优: 阈值 10x→20x + MIN_DIFF_LINES_FOR_DETECTION 100→200 + MIN_MSG_LINES_THRESHOLD 3→2 + 14 sprint workflow commit type prefix whitelist (fix(etl)/fix(test)/fix(etl+git)/fix(backend)/fix(frontend)/feat(etl)/feat(backend)/feat(frontend)/chore(sprint)/docs(sprint)/chore(frontend)/chore(etl)/refactor(etl)/refactor(backend)) + hook 提示优化 (Sprint 120 优先级 4 条修复建议) + 5 case regression (留尾治理 sprint 修 Sprint 117+118+119 期间 4 次 --no-verify hotfix bypass 真因, 误报率 4/9=44% → 0%, 跟 Sprint 32.3 a9b1d91 教训兼容保留简单 msg 拦截, 1 file +38/-10 行 + 1 new test file 5 case PASS, pytest 837/23/0 baseline +5 vs Sprint 119 832, 累计 60 sprint 0 debt 持续 (+1 vs Sprint 119 59), L4.x 22 stable 0 新增, VERSION 0.4.14.157 不变, /review 0 finding 范围严格对应 commit-msg drift hook 调优 0 越界))
**当前债数**: 0 条 (Sprint 120 持续, Sprint 117 修 #D11-#D14 4 项 Sprint 120 修 commit-msg drift hook 误报率 44% → 0% 持续, 当前债数仍 0)
**跨 sprint 留尾**: 0 条 (Sprint 120 修 commit-msg drift hook 调优真治本, Sprint 117 修 #D11-#D14 4 项, Sprint 116 留尾全闭环; D1 50m-scale 为按 30M 数据量触发的推后决策, 不计当前债)
