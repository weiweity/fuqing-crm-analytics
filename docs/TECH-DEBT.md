...
**最后更新**: 2026-07-02 (Sprint 187 — CI #500 / #499 / #497 累计 3 sprint CI 复发 root 因 100% 治根 + L4.41 subprocess PYTHONPATH 绝对路径永久规则 + fix_pattern #72 沉淀. pytest 893/73 skip 持续, ruff 0 errors, main HEAD `4be74bc + Sprint 187 squash`, 累计 116→**117 sprint 0 debt** (Sprint 187 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +10 sprint), VERSION 0.4.14.29→**0.4.14.30** (Sprint 187 CI root 因 + L4.41 永久规则 + 累计 18 次 /document-release 真治本, 跟 Sprint 179/181/182/183/184/185/186 模式 stable, 累计 9 次 /document-release bump 持续), L4.x 34→**35 stable** (新增 L4.41 subprocess 注入 env[PYTHONPATH] 必须用 str(PROJECT_ROOT) 绝对路径不 inherit 父进程), 11 hook 闭环 (跟 Sprint 185 一致). 当前债数仍 0, 跨 sprint 留尾仍 0, 已修复累计 60+ 条 (Sprint 187 治本跨 sprint 5+ 复发真因真发现, 跟 Sprint 60+ 0 debt stable 模式 stable)
