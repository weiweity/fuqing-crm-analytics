...
**最后更新**: 2026-07-01 (Sprint 183 — WorkBuddy /ad-hoc-query 优化 + SKILL.md v2.2 顶部 3 段 (执行路径强制 + 需求-工具映射速查表 + 锁冲突 graceful fallback) + L4.36 禁停 uvicorn 永久规则 + daily-gsv-multi_period 第 11 个 MCP tool 新子命令 + QA 抓到的 3 个真 bug 治本 (双 registry 注册丢失 / query name hyphen 化 / 4 unused imports) + 删 2 个临时脚本 + 5 个端到端锁回归 test + Sprint 182 /document-release v0.4.14.25 续集. pytest baseline **78** (含 Sprint 183 +9 cases + 5 端到端锁回归, 累计 +69 vs Sprint 182), main HEAD `3023a07` + origin/main 0 drift, 累计 112→**113 sprint 0 debt** (Sprint 183 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +6 sprint), VERSION 0.4.14.25→**0.4.14.26** (Sprint 183 WorkBuddy /ad-hoc-query 优化 + SKILL.md v2.2 + L4.36 禁停 uvicorn + daily-gsv-multi-period 新子命令 + 5 端到端锁回归 + 累计 14 次 /document-release 真治本, 跟 Sprint 182 模式 stable, 累计 5 次 /document-release bump 持续), L4.x 29→**30 stable** (新增 L4.36 ad-hoc-query 禁停 uvicorn, Sprint 183 沉淀 fix_pattern #68-#69 累计 2 模式), 11 hook 闭环 (7 Claude Code + 4 git hooks, 持续 stable). 当前债数仍 0, 跨 sprint 留尾仍 0, 已修复累计 60+ 条 (Sprint 183 没新增任何债, 仅 Sprint 181 治本遗留 25 个 test 都有 `os.chdir(tmp)` 风险等真业务触发)
