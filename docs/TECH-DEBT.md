...
**最后更新**: 2026-06-25 (Sprint 111 retention 7→2 天滚动 + KEEP_MIN 1→2 + 3 plist 修复 (留尾治理 sprint, 跟 Sprint 99+100+101+102+103+104+105+110 模式 一致), 6 文件 +220/-9 行, pytest 825/23/0 持续 (+6 vs Sprint 110 819), 累计 57 sprint 0 debt 持续, L4.x 22 stable 0 新增, VERSION 0.4.14.157 不变, /review 2 CRITICAL defer 加 #D5 + #D6 留尾 Sprint 112+ 一起修 (L4.21 反 sprint 自我反馈闭环遵守))
**当前债数**: 0 条 (Sprint 110 持续, Sprint 111 /review 2 CRITICAL defer 标 #D5 + #D6 留尾 Sprint 112+ 真 refactor sprint 一起修, 当前债数仍 0 因为 defer 不算当前债)
**跨 sprint 留尾**: 2 条 (Sprint 111 新增 #D5 cleanup_backups.py 缺 8 safety check + #D6 cleanup_backups.py 缺 pytest test, 留尾 Sprint 112+ 真 refactor sprint 一起修; D1 50m-scale 为按 30M 数据量触发的推后决策, 不计当前债)
...
- 📋 **#D5 cleanup_backups.py 缺 8 项 safety check (Sprint 111 /review testing specialist CRITICAL defer)**: Sprint 111 引入 scripts/etl/cleanup_backups.py L4.7 Python 端口 (替代 /bin/bash macOS sandbox deny), 1:1 port of cleanup_backups.sh 仅保留 mtime + BACKUP_KEEP_MIN + soft fail, 缺 backup_duckdb.py:_prune_old_backups() 的 8 项 safety check (lsof 0 fd + zstd magic + size>0 + not compressed_path + sorted by mtime desc + soft fail). Sprint 111 scope creep 风险 (1 真业务 sprint + 0 越界原则 L4.21), 留尾 Sprint 112+ 真 refactor sprint 一起修 (抽 shared `_prune_with_safety()` 函数 + cleanup_backups.py + backup_duckdb.py 复用, 0 代码 duplicate, 1 真业务触发)
- 📋 **#D6 cleanup_backups.py 缺 pytest test (Sprint 111 /review testing specialist CRITICAL defer)**: Sprint 111 引入 scripts/etl/cleanup_backups.py 117 行新模块, 0 pytest test (现有 test_backup_duckdb.py 4 case 覆盖相同算法 _prune_old_backups, 但 cleanup_backups.py 是独立 module 100% 代码路径未覆盖: mkdir lock + FQ_BACKUP_RETENTION_DAYS env override in cleanup_backups + FQ_BACKUP_KEEP_MIN env override + before/after stats + log file append + disk_free reporting). Sprint 111 scope creep 风险 (1 真业务 sprint + 0 越界原则 L4.21), 留尾 Sprint 112+ 跟 #D5 一起修 (加 test_sprint111_cleanup_backups.py 5 case: retention env override + KEEP_MIN env override + lock dir skip concurrent + soft fail on unlink fail + log file append)
