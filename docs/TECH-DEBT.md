...
**最后更新**: 2026-07-05 (Sprint N+1 业务方访谈 PDF 需求文档 完成 + Wave 1 跨 sprint plan Sprint N+1 to N+5 跨 sprint 续期 0 commit 收口 — 你 7/5 跟 1 个运营经理 (跟业务组开发直接对, 跟 Q21 联系方式 1:1 stable) 完成 21 个 业务方答复 (Q1-Q20 + Q21) PDF 需求文档. 5 校准 SCENARIOS (跟 Sprint N+2 SCENARIOS 1:1 stable 沿用): s02 加 R/F/M 区间 + 导出订单号 / s03 加 30 指标 + 达播/微信/货架渠道列表 / s04 语义校准 "老客品类回流反向追溯" / s06 加 product 维度 + 自由自定义时间 / s05 + s07 降级 (业务方不重点). 5 不变 SCENARIOS: s01/s08/s09/s10 1:1 stable 沿用 (跟 RFM_DEFINITIONS.md 业务 SSOT 1:1 stable 校准). 业务方核心画像 (跟 21 答复 1:1 stable): 1) 正确性 > 速度 (Q2/Q4/Q9 1:1 stable 沿用) 2) top 频繁 s02 + s09 + s10 (Q11 1:1 stable) 3) 期望 P95 <5s (Q12) + <2s 满意 (Q17) 4) 双写期接受 (Q18 "数据不能错") 5) 灰度接受 (Q19 "不能影响业务") 6) Go/No-Go 拍板 接受 (Q20 "我跟业务组对结果") 7) 联系方式 不要 (Q21 "开发对"). 需求文档 `docs/sprints/SPRINT-N+1-BUSINESS-INTERVIEW-REQUIREMENTS.md` (~290 行, 跟 clickhouse-poc-decision-memo.md §3.1 deliverable 1:1 stable + RFM_DEFINITIONS.md 业务 SSOT 1:1 stable 沿用). 跟 Sprint 60+ 累计 +40 sprint 0 debt stable 1:1 stable 沿用 L4.42 + L4.55 + L4.56 + L4.57 + L4.59 + L4.14 amend 物理限制 1:1 stable 接受 1 commit drift. 累计跨 sprint plan 收口累计 8 件 (N+1 阶段 1 PDF 需求 1:1 stable + N+2 shipped + N+3 cluster handoff doc ✅ + N+4 ETL handoff doc ✅ + N+5 Go/No-Go 决策模板 ✅ + 跨 sprint 留尾 7 件 0 commit 续期). 后续 Sprint N+1 W2 DuckDB 128GB 性能基线 (跟 R8 wall_min 10.8min 1:1 stable 治本延伸) + Sprint N+3 cluster 真 benchmark (跟 §4.1 校准) + Sprint N+4 ETL 实施 (跟 §4.2 校准) + Sprint N+5 Go/No-Go 拍板 (跟 Q20 1:1 stable). 累计 Sprint 60+ 0 debt stable 139 sprint (跨 +35 sprint).
