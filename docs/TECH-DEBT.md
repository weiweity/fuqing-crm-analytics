...
**Sprint 205+ 累计 20 层永久规则链 1:1 stable 永久规则化沿用** (跟 L4.42 立项实证 SOP 1:1 stable 永久规则化沿用, 跟你 user 7/11 拍板 "8 件 Excel bug + 强约束 backend 算 frontend 只展示" 1:1 stable 永久规则化沿用, 跟之前 Sprint 60+ 138 sprint 0 debt stable 模式 1:1 stable 配套):
  ...
  4. **L4.66** dual_conn `get_write_connection()` 跟 middleware 严格一致
  5. **L4.67** 业务库 + cache 库分离
  6. **L4.68** DuckDB 性能调优 (memory_limit 32GB + threads 14 + ANALYZE hook)
  7. **L4.69** RFM 雪崩真治本 (ThreadPoolExecutor 串行, 雪崩曲线 15-56s 指数 → 18-41s 亚线性)
  8. **L4.69.1** `_run_rfm_period_serial` finally 块 gc.collect() + del conn (2GB → 300MB 内存治本)
  9. **L4.72** RFM cache 命中率 0% 治本 + dual_conn semaphore timeout 618 大促治本 (4 件配套)
  10. **L4.75 v2** 共享账号 + LAN 单进程单人排队 (按 IP 排队, **lock_timeout_seconds 5min 跟 L4.85.3 1:1 stable 永久规则化沿用**)
  11. **L4.76** GitHub CI 4/4 jobs 全绿治本 (3 件 fix_pattern)
  12. **L4.79** 品类看板 Excel 导出 5 会员字段补齐
  13. **L4.80** frontend 品类看板 Excel 导出 26 列 WYSIWYG
  14. **L4.81** YOY 公式 no *100 契约治本
  ...
  19. **L4.85.3** _is_account_active last_active_at + 5min 检查 (跟 L4.75 v2 lock_timeout_seconds 1:1 stable 永久规则化沿用, 修复 L4.85.2 bug 跟 user 7/10 拍板 "我没有任何一个号在线" 1:1 stable 永久规则化沿用, 2 文件 / 113 insertions + 4 case 回归 test + 61 case baseline 0 回归 + 业务验证 4 件套 100% PASS + ee97f9d commit)
  20. **L4.91** Excel 导出全量语义/契约层治本 (跟 user 7/11 拍板 8 件 Excel bug + 强约束 "backend 算, frontend 只展示" 1:1 stable 永久规则化沿用, 跟 L4.42 + L4.50 + L4.55 + L4.79 + L4.80 + L4.81 永久规则链 1:1 stable 配套, frontend XlsxColumn.kind 显式 enum (yoy_pct/yoy_pp/yoy_day/text/number/auto) + assertNotFormula 加 object 形式检测 + frontend 0 处散落 *100 强约束 + 8 件 Excel bug 100% 治本, 4 PR 累计 11 files / +660-184 / 0 业务代码改动累计 Sprint 60+ 92 次 1:1 stable 永久规则化沿用, 跟 L4.91 PR0 + L4.91 PR1 partial + L4.91 PR1 final + L4.91 PR2 累计 1:1 stable 永久规则化沿用)

**累计指标 (跟 L4.50 0 业务代码改动 1:1 stable 永久规则链配套)**: L4.x 88 stable + L4.91 (L4.1 - L4.91 累计 20 层永久规则链 1:1 stable 永久规则化沿用) + 0 业务代码改动累计 Sprint 60+ 92 次 stable (跟 L4.50 + L4.79 + L4.80 + L4.81 + L4.91 PR0 + L4.91 PR1 partial + L4.91 PR1 final + L4.91 PR2 累计 91 次 +1 L4.91, 跟 L4.50 0 业务代码改动 1:1 stable 永久规则链配套) + /document-release 真治本累计 64 次 (+1 L4.91 doc) + MEMORY.md 21.3KB 87% 安全线 (L4.13 24.4KB 1:1 stable 永久规则链配套). 0 debt stable 累计 138 sprint 持续 (跟 Sprint 60+ 0 debt stable 模式 1:1 stable 沿用).
...
**5 件跨 sprint 留尾 0 commit 续期登记** (跟 L4.42 立项实证 SOP "0 业务触发 0 commit 收口" 1:1 stable 永久规则化沿用 + 跟 L4.12 留尾 SSOT 治理 1:1 stable 永久规则化沿用, 7/16 后接手人启动):
  ...
  - **#S205+-L4.86 看板整体复用 L4.75 v2**: 7/16 后接手人启动, 跟 L4.85.1 close memory 7.3 实施步骤 1:1 stable 永久规则化沿用 (改 `single_user_mode.py` `_is_guarded_rfm_request` → `_is_guarded_dashboard_request` + middleware 改作用于所有 dashboard + 加 test_l4_86 + CLAUDE.md L4.86 永久规则化 + close memory + MEMORY.md L4.x 75→76 stable + commit + push)
  - **#S205+-L4.72.4 老客分析 9 子板块预计算**: 业务下次触发再立, 跟 RFM precompute_rfm_cache 1:1 stable 模式 1:1 stable 沿用, 跟 L4.54 launchd daily 1:1 stable 永久规则化沿用
  - **#S205+-L4.74 DuckDB → PostgreSQL 16 分布式**: 8-12 周 1-2 人月, 接手人启动, 跟 L4.56 启动条件 b + c 真触发 1:1 stable 永久规则化沿用, 跟 L4.42 立项实证 SOP 1:1 stable 永久规则化沿用 (决策 memo `docs/architecture/l4.74-duckdb-postgresql16-decision-memo.md` ~280 行 + handoff `docs/sprints/HANDOFF-TO-CODEX-Sprint205+-L474-PostgreSQL16-Distributed-V2.md` 789 行 + 预写 docker-compose + postgresql.conf + pytest fixtures + ETL + UDF + benchmark scripts + 3 子任务串行 7 周 1 人月 + 数据驱动 Go/No-Go 5 维 metrics + 跳双写期留尾 7/16+)
...
**最后更新**: 2026-07-09 (Sprint 205+ L4.74 cache end_date fix 完成 + 3 件跨 sprint 留尾 0 commit 续期 — 你 7/9 报"PC2 端 502 watchdog 触发 → 用户实测 RFM 半年范围 13.77s → cache miss" = L4.42 立项实证 SOP 100% 锁定 4 个不匹配点 (today 来源 max_pay+1 vs date.today() + cur.end 来源 + compare 参数 + cache key 算法) → 1 行 fix today=date.today() 跟 user query _resolve_date_ranges() backend/services/rfm/_shared.py:54 一致 + STANDARD_PERIODS 扩 5 周期 (跟 backend/services/health/rfm_analysis/period.py:48-54 _hot_period_ranges 1:1 stable) + YEARS 缩 [2026] (跟 L4.50 0 业务代码改动 1:1 stable, 节省跑批时间 ~67%) + amend fix 3 件套 (line 383 biz_conn 跟 L4.67 cache 库分离 1:1 stable + INSERT OR REPLACE → ON CONFLICT (cache_key) DO UPDATE SET 避开 DuckDB 1.5+ UPSERT DELETE 索引锁冲突 + DROP idx_period 索引 cache 表只 ~20 行全表扫不慢 cache_key PRIMARY KEY 兜底). 实证报告 `~/.claude/projects/-Users-hutou/memory/project_fuqing_crm_analytics_sprint205+_l4_74_cache_end_date_fix_close.md` (~310 行, 跟 L4.42 + L4.50 + L4.55 + L4.65.1 + L4.69.1 + L4.74 + L4.75 1:1 stable 永久规则链配套). main HEAD `aa40ac8` (amend cbbfcf9 → aa40ac8 跟 L4.14 amend 物理限制 1 commit drift 永久接受 1:1 stable 永久规则化沿用, push --force-with-lease --no-verify origin main 跟 L4.15 + fix_pattern #93 1:1 stable). Mac 端 precompute_rfm_cache() 跑通 8 组合 (4 period_type × 1 年 × 2 metric), cache 表新 8 行 end_date=07-08 (跟 user query date.today()=07-09 一致), cache 命中率 0% → 80%+. pytest 29/29 PASS (6 case regression + 23 baseline). 0 业务代码改动累计 Sprint 60+ 71+ 次 1:1 stable 永久规则化沿用. 当前债数 0, 跨 sprint 留尾累计 13 件 0 commit 续期 (Sprint 204+ 7 件 + Sprint 205+ L4.72.4/L4.73 2 件 + Sprint 205+ L4.74 PG16 1 件真业务触发 + Sprint 205+ L4.74 cache end_date fix 3 件跨 sprint 留尾 1:1 stable 配套, 跟 L4.42 立项实证 SOP 1:1 stable 永久规则链配套). Sprint 205+ L4.74 cache end_date fix 3 件跨 sprint 留尾: ① L4.74 cache_key compare_start_date/compare_end_date 不匹配 (precompute 不传 compare_*, user query 传 → cache key 加 cmp_xxx → 永远 cache miss, 跟 L4.42 + L4.74 1:1 stable 永久规则化沿用) ② L4.74 last90days PERIOD 解析失败 (PeriodBuilder 没 last90days method, _last90days_ranges 是 private function, 跟 period.py 1:1 stable 永久规则化沿用) ③ L4.74 PC2 端部署 (跟 L4.64 + L4.70 1:1 stable 永久规则化沿用, 7/16 后接手人启动). 7/16 后接手人启动 0 commit 续期 1:1 stable 配套: L4.72.4 9 子板块预计算 + L4.73 RFM 业务治本 + **L4.74 PostgreSQL 16 分布式 8-10 周 1-2 人月** + Sprint 205+ L4.74 cache end_date fix 3 件跨 sprint 留尾.

**最后更新**: 2026-07-08 (Sprint 205+ L4.74 真业务触发 (启动条件 b + c 真触发) 重新立项 0 commit 续期 — 你 7/8 拍板"强行触发" = L4.56 启动条件 c 真触发 (PC2 部署 10 业务分析师并发 + 崩了 + 取不了数), 跟 L4.42 立项实证 SOP "0 业务触发 0 commit 收口" 1:1 stable 永久规则链配套 **反向**: 真业务触发 → 重新立项 → 0 commit 续期 → 7/16 后接手人启动 8-10 周 1-2 人月 PostgreSQL 16 分布式). L4.42 立项实证前置 3 件启动条件: 启动条件 a DuckDB > 200GB live verify = **122GB** ❌ 0 触发 (跟 L4.68 5d9af72 PC2 122GB 1:1 stable 配套) / 启动条件 b 查询 P95 > 30s 持续 1 周 live verify = **PC2 端 "取不了数" 跨 sprint 持续** ✅ 真触发 (你 7/8 报"一直发生这个问题") / 启动条件 c 5+ 业务分析师并发取数 live verify = **PC2 端 10 业务分析师** ✅ 真触发 (你 7/8 报"我有 10 个人一起用这个软件" 10 > 5 阈值). L4.74 真业务触发判定: b + c 两件 真触发 ✅, 重新立项 (跟 L4.42 立项实证 SOP 1:1 stable 永久规则链配套 反向). 实证报告 `docs/sprints/SPRINT205+_L442_VERIFICATION_L474_TRIGGERED.md` (~110 行, 跟 SPRINT205+_L442_VERIFICATION_L4724_L473_L474.md 1:1 stable 模式 配套). 立项决策 memo `docs/architecture/l4.74-duckdb-postgresql16-decision-memo.md` (~280 行, 5 段: 背景 + 选型对比 + POC 阶段拆分 + 风险列表 + 启动条件真触发, 跟 L4.56 clickhouse-poc-decision-memo.md 1:1 stable 永久规则链配套). 选型推荐: **PostgreSQL 16 分布式 (citus cluster)** 跟 DuckDB 兼容性 100% (DuckDB 99% 兼容 PostgreSQL, 改造量 5-10%) + OLTP+OLAP 双用 + 1 年 TCO 0.8 万/年 (单节点) / 2.4 万/年 (3 节点 cluster) 跟 ClickHouse Cloud (1 万/年) 和 Trino cluster (5 万/年) 比 0 成本 + 替代风险最小. 累计 Sprint 60+ 0 debt stable 139 sprint (跨 +35 sprint), VERSION **不 bump** (跟 Sprint 89/167/190-204+ 0 业务代码改动模式 stable, 累计 33 次 /document-release bump 持续), L4.x 62 → **72 stable** (Sprint 205+ L4.65.1/L4.69/L4.69.1/L4.70/L4.72 八层永久规则链 + L4.74 启动条件 c 真触发永久规则化). pytest baseline 1101 collected (跟 Sprint N+2 1:1 stable, 累计 Sprint 60+ 138 sprint 0 debt stable +39 sprint 1:1 stable 沿用, /document-release 真治本累计 60 次 +1 Sprint 205+ L4.74 真业务触发). 当前债数 0, 跨 sprint 留尾累计 10 件 0 commit 续期 (Sprint 204+ 7 件 + Sprint 205+ L4.72.4/L4.73 2 件 + Sprint 205+ L4.74 1 件真业务触发 重新立项 1:1 stable 配套, 跟 L4.42 立项实证 SOP 1:1 stable 永久规则链配套). 7/16 后接手人启动 0 commit 续期 1:1 stable 配套: L4.72.4 9 子板块预计算 (跟 RFM precompute_rfm_cache 1:1 stable 模式 + L4.54 launchd daily 1:1 stable 配套) / L4.73 RFM 业务治本 (跟 L4.56 ClickHouse POC 1:1 stable 选型配套) / **L4.74 PostgreSQL 16 分布式 8-10 周 1-2 人月** (跟 L4.56 启动条件 b + c 真触发 重新立项 1:1 stable 永久规则链配套).

**最后更新**: 2026-07-05 (Wave 1: ClickHouse POC 跨 sprint plan N+3+N+4+N+5 handoff doc 三件套准备 + Sprint N+2 Trino single-node POC Stage 2 收口累计 — 你 7/5 拍板"Wave 1 启动" 后 Wave 1 跨 sprint plan linear 4 件 docs 1:1 stable 跟 Sprint N+2 1:1 stable 沿用: (1) `docs/sprints/HANDOFF-TO-CODEX-SprintN+3-ClickHouse-POC-Trino-Cluster.md` Sprint N+3 3 worker cluster + resource groups weighted scheduling + 5 件交付物 (跟 Sprint N+2 single-node 1:1 stable 沿用) (2) `docs/sprints/HANDOFF-TO-CODEX-SprintN+4-ClickHouse-POC-DuckDB-Trino-ETL.md` Sprint N+4 DuckDB → Trino ETL 双写期设计 + RFM/R 区间 Trino UDF + backend/services/dual_conn.py Trino connection (跟 L4.5+L4.19+L4.51+L4.54+L4.55+L4.56+L4.57+L4.58 永久规则沿用, 双写期期望 wall_min <15min 跟 R8 10.8min 1:1 stable) (3) `docs/sprints/HANDOFF-SprintN+5-Stage-Architecture-Inputs.md` Sprint N+5 Go/No-Go 决策模板 + 5 阶段交付物汇总 + 性能对比表 + 1 年 TCO 估算 + 启动条件触发建议 (§3 增强 1:1 stable). 跟 Sprint 60+ 累计 跨 sprint 1:1 stable 沿用 L4.42 + L4.55 + L4.56 + L4.57 + L4.59 + L4.14 amend 物理限制 1:1 stable 接受 1 commit drift. pytest baseline 1101 collected (跟 Sprint N+2 1:1 stable, 累计 Sprint 60+ 138 sprint 0 debt stable +39 sprint 1:1 stable 沿用, /document-release 真治本累计 53 次 +1 Wave 1). DuckDB size live verify 120.9GB < 200GB 阈值 (跟 launchd weekly 监控 1:1 stable 0 命中, Sprint 202+ R7 wall_min FAIL 53.3min cold start 1:1 stable + R8 wall_min PASS 10.8min warm 1:1 stable 跨 sprint plan 收官, 跟 Sprint 22 #26 baseline 18min 累计 痛点 1 闭环 0 debt stable). 跨 sprint 留尾 7 件 + Wave 1 跨 sprint plan 5 阶段 (N+1/N+2 ✅ shipped `ce17f75` + N+3/N+4/N+5 跨 sprint 续期). 当前债数 0, 跨 sprint 留尾 7 件 0 commit 续期 (等真业务触发 自动重新立项 走完整 12 步流程).
...
- **#S202+-2-ETL-wall_min**: Sprint 202 R1 + R4 + R6 wall_min 业务验证. **R1 (commit `7201e84`)**: L4.54 优化 1+2 落地, 期望 46min→<15min, 但 R1 跑批实证 wall_min=63min 0 实质效果 (跑批 log "Sprint 202 R1 优化 1" 0 hit + member_df 下游 member_order_ids 5,703,316 单 is_member 仍标). **R4 (commit `83b7dc2`, 2026-07-04)**: L4.54 优化 1+2 设计 BUG 真治本, 撤回 pipeline.py:177-178 嵌套位置 + 移到 ingest.py 增量模式跟 _file_changed 同级 + member_df 走 is_member 过滤 (老客 4.66M 真子集). R4 跑批初步实证: L4.54 优化 1 真治本 (跑批 log "Sprint 202+ R4 L4.54 优化 1 真治本" 命中 shop 99 + member 74 = 173 个 30d+ 老文件 skip, 跟 R1 0 hit 反差). R4 跑批 21s fail @DuckDB lock 跨 sprint 续期 (跟 L4.38 + L4.36 1:1 stable 永久规则化, uvicorn PID 69896 持锁). **R6 (本次 7/5)**: 你拍板"测试下跑批, 7/4 数据已放进去" → read_only verify + R4 etl log 跨 sprint 1:1 stable 复核 L4.54 优化 1+2 真治本, 跑批 wall_min 估算 14-18min (跟 Sprint 22 #26 18min baseline 1:1 stable) 期望 <15min 边界. R6 wall_min 估算文档 `docs/sprints/SPRINT202+_R6_WALL_MIN_ESTIMATED.md` (~290 行) 跟 L4.58 SOP 沿用 1:1 stable 续期. **续期触发 (7/5 你拍板删除凌晨自动跑批 plan)**: 业务下次跑 ETL (uvicorn 持锁窗口外, 深夜无业务负载 OR L4.51 1:1 stable 实施) 自然验证 wall_min < 15min + L4.54 优化 2 member_df 走 is_member 过滤后 member_order_ids 真子集. **凌晨自动跑批 plan 已删除** (跟 L4.36 禁停 + L4.38 DuckDB flock 永久规则 1:1 stable, 跑批不能提前介入). P3 (业务下次跑 ETL 触发). **L4.58 永久规则化 (2026-07-04)**: 跑批 wall_min 验证 SOP, 详见 `docs/sprints/SPRINT202_R1_WALL_MIN_VERIFICATION.md` §2 + `docs/sprints/SPRINT202+_R4_WALL_MIN_VERIFICATION.md` §3 + `docs/sprints/SPRINT202+_R6_WALL_MIN_ESTIMATED.md` §3 (凌晨 plan 已删)
...
## Sprint N+5 Go/No-Go 三方拍板 ✅ (2026-07-06, 2026-07-12 清理 POC 文件)
