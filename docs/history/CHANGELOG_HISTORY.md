...
- **Sprint 181.1 硬编码 macOS 绝对路径治本**: `test_claude_hooks.py:807-810` 硬编码 `/Users/hutou/Desktop/ai-engineering/历史项目/fuqin-date/...` macOS 路径, CI Linux runner 抛 `FileNotFoundError` fail. 治本: `Path(__file__).resolve()` 跨平台 (macOS / Linux / Windows 都 work). 跟 Sprint 179.1 实战 fix 模式 #58 互补 (漏查"绝对路径"维度)
