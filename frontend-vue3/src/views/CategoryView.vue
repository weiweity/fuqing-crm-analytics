...
// L4.79 + L4.80 + L4.81 + L4.91 WYSIWYG 25 列扩列 (跟 frontend allColumns 1:1 stable, 全店+老客+新客 groups + 会员占比 + 会员渗透率)
// L4.91 PR1 治本 Bug #3: YOY 列用 kind enum 显式分支 (跟 backend L4.81 契约 1:1 stable 永久规则化沿用)
// 1 + 9 (全店) + 8 (老客) + 8 (新客) = 26 列. 全店 group 多 1 列 会员占比 YOY, 跟 frontend allColumns (line 395-453) 1:1 stable 沿用
// - absolute YOY (gsv/users/aus) → kind='yoy_pct' (raw *100 = % 后缀, 跟 L4.81 yoy_absolute 1:1 stable 永久规则化沿用)
...
const allCompactXlsxColumns = computed<XlsxColumn[]>(() => [
  ...
  { header: '全店-会员占比', key: 'member_ratio', kind: 'number', width: 12, numFmt: '0.0%' },
  { header: '全店-会员占比 YOY (pp)', key: 'member_ratio_yoy', kind: 'yoy_pp', width: 12 },
  { header: '全店-用户数', key: 'users', kind: 'number', width: 12, numFmt: '#,##0' },
  { header: '全店-用户数 YOY%', key: 'users_yoy', kind: 'yoy_pct', width: 12 },
  { header: '全店-AUS', key: 'aus', kind: 'number', width: 12, numFmt: '¥#,##0' },
  { header: '全店-AUS YOY%', key: 'aus_yoy', kind: 'yoy_pct', width: 12 },
  { header: '全店-会员渗透率', key: 'member_penetration', kind: 'number', width: 12, numFmt: '0.0%' },
  ...
  { header: '老客-AUS YOY%', key: 'old_aus_yoy', kind: 'yoy_pct', width: 12 },
  ...
  { header: '新客-AUS YOY%', key: 'new_aus_yoy', kind: 'yoy_pct', width: 12 },
])
