...
watch([mFlowQueryKey, compareQueryParams], () => { mFlowAutoFetch.value = false }, { deep: true })
function onMFlowQueryClick() {
  triggerManualQuery(mFlowAutoFetch, mFlowFetching, mFlowRefetch)
}

const {
  data: mFlowData,
  isLoading: mFlowLoading,
  isFetching: mFlowFetching,
  error: mFlowError,
  refetch: mFlowRefetch,
} = useQuery({
  queryKey: mFlowQueryKey,
  queryFn: ({ signal }) => fetchRFMMFlow(
    { ...toValue(mFlowQueryParams), ...toValue(compareQueryParams) },
    signal,
  ),
...
const mFlowColumns = computed<DataTableColumns<RFMMFlowRow>>(() => {
  ...
  return [
    ...
    },
    {
      ...
      width: 110,
      align: 'center',
      render: (r: RFMMFlowRow) => h(YOYGuard, { value: r.yoy_repurchase_gsv_ratio_ppt, unit: 'pp', styled: true}),
    },
...
const mFlowXlsxColumns = computed<XlsxColumn[]>(() => {
  ...
  return [
    ...
    { header: `${yr}同比历史人数`, key: 'yoy_hist_users', kind: 'yoy_pct', width: 12, numFmt: '0.0%' },
    { header: `${yr}同比回购人数`, key: 'yoy_repurchase_users', kind: 'yoy_pct', width: 12, numFmt: '0.0%' },
    { header: `${yr}同比回购率`, key: 'yoy_repurchase_rate', kind: 'yoy_pp', width: 12, numFmt: '0.0%' },
    { header: `${yr}同比回购GSV`, key: 'yoy_repurchase_gsv', kind: 'yoy_pct', width: 12, numFmt: '0.0%' },
    { header: `${yr}同比回购GSV占比`, key: 'yoy_repurchase_gsv_ratio_ppt', kind: 'yoy_pp', width: 14, numFmt: '0.0%' },
...
<template>
  <div class="m-interval-tab">
    ...
    <div class="bi-card p-4 mb-4">
      ...
      <ErrorState v-if="mFlowError" :message="(mFlowError as Error).message" @retry="onMFlowQueryClick" />
      <LoadingState v-else-if="mFlowLoading" />
      <div v-else-if="!mFlowAutoFetch" class="manual-query-guide">
        <ManualQueryButton :loading="mFlowFetching" @click="onMFlowQueryClick">查询 M 区间数据</ManualQueryButton>
