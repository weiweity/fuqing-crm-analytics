...
const { data: dailyData } = useQuery({
  queryKey: computed(() => ['product-assets-daily', { ...toValue(dailyQueryParams) }]),
  queryFn: () => {
    const p = toValue(dailyQueryParams)
    return fetchProductAssets(p.weeks, p.days)
  },
  staleTime: 5 * 60 * 1000,
})
...
const { data: weeklyData, isLoading, error, refetch } = useQuery({
  queryKey: computed(() => ['product-assets-weekly', { ...toValue(weeklyQueryParams) }]),
  queryFn: () => {
    const p = toValue(weeklyQueryParams)
    return fetchProductAssets(p.weeks, 0)
  },
  staleTime: 5 * 60 * 1000,
})
...
<template>
  <div>
    <LoadingState v-if="isLoading" description="正在加载单品资产数据..." />
    <ErrorState v-else-if="error" :message="String(error)" @retry="refetch" />
    <EmptyState v-else-if="!weeklyData?.products?.length" description="暂无单品资产数据" />
    ...
    <div v-else>
      ...
      <div class="bi-card p-4 mb-6">
        <div class="mb-0.5">
          ...
          <p class="text-[11px] text-slate-500">DMP单品人群日度流转 — 各产品资产总量变化</p>
        </div>
        <LineChart
      ...
      <div class="flex items-center justify-end mb-2">
        <ExportToolbar
          :filename="`单品资产_周维度_${props.weeks}周`"
          :columns="productAssetsXlsxColumns"
