...
const { data, isLoading, error, refetch } = useQuery({
  queryKey: computed(() => ['product-assets', { ...toValue(queryParams) }]),
  queryFn: () => {
    const p = toValue(queryParams)
    return fetchProductAssets(p.weeks)
  },
  staleTime: 5 * 60 * 1000,
})
...
<template>
  <div>
    ...
    <EmptyState v-else-if="!data?.products?.length" description="暂无单品资产数据" />
    ...
    <div v-else>
      ...
      <div class="bi-card p-4 mb-6">
        <div class="mb-0.5">
          ...
          <p class="text-[11px] text-slate-500">DMP单品人群周度流转 — 各产品资产总量变化</p>
        </div>
        <LineChart
