...
<template>
  <div class="bi-card bi-card-hover px-4 py-3 transition-all duration-200 h-full">
    <div class="flex items-center justify-between">
      ...
      >
        <template v-if="unit === '%'">{{ change > 0 ? '↑' : change < 0 ? '↓' : '' }}{{ change !== 0 ? Math.abs(change).toFixed(2) + '%' : '0.00%' }}</template>
        <template v-else>{{ change > 0 ? '↑' : change < 0 ? '↓' : '' }}{{ change !== 0 ? Math.abs(change * 100).toFixed(2) + 'pp' : '0.00pp' }}</template>
      </span>
    </div>
    <div class="mt-1.5">
