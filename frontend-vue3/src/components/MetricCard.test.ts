...
function getChangeText(wrapper: ReturnType<typeof mount>, label: string): string {
...
describe('MetricCard YOY/pp display (Sprint 11 修)', () => {
  it('integer ratio 0.14 (% unit) 显示 "14%" (trim trailing .00)', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↑14%')
  })
  ...
  it('integer pp 0.10 (pp unit) 显示 "10pp" (trim trailing .00)', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↑10pp')
  })
  ...
  it('non-integer ratio 0.8061 显示 "80.61%" (保留 2 位小数)', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↑80.61%')
  })
  ...
  it('non-integer pp 0.0358 显示 "3.58pp" (保留 2 位小数)', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↓3.58pp')
  })
  ...
  it('half-up 0.145 rounds to "14.5%" (不是 "14.49%" toFixed bug)', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↑14.5%')
  })
  ...
  it('0 显示 "0%" (不显示 "0.00%")', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('0%')
  })
  ...
  it('negative change 显示 ↓ 箭头', () => {
    ...
    expect(getChangeText(wrapper, 'change')).toBe('↓7%')
  })
})
