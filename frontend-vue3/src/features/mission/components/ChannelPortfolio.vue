...
<template>
  <article class="glass-panel channel-panel">
    ...
    <div class="channel-table" role="table" aria-label="渠道客户资产对比">
      ...
      <div v-for="(item, index) in metrics" :key="item.channel" class="channel-row" role="row">
        <span class="channel-name" role="cell"><i>{{ String(index + 1).padStart(2, '0') }}</i><strong>{{ item.channel }}</strong></span>
...
.leader-note span { padding: 7px 9px; border: 1px solid var(--sm-line); color: var(--sm-muted); background: rgba(255,255,255,.025); font-size: 9px; }
...
.channel-name i { color: rgba(211, 195, 232, .35); font: 500 9px/1 var(--sm-font-mono); }
...
.volume-cell > span { display: block; height: 3px; background: rgba(255,255,255,.06); }
.volume-cell > span i { display: block; height: 100%; background: linear-gradient(90deg, var(--sm-purple), var(--sm-lilac)); box-shadow: 0 0 12px rgba(211, 195, 232, .2); }
