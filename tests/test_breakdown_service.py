...
class TestRIntervalBoundaries:
    """验证 _get_r_interval_current_distribution 的 R 区间分桶正确性"""

    def test_r_interval_bucketing_correct(self, monkeypatch):
        """用户按 recency 正确分入 R 区间，按购买次数分入 F 段"""
        import duckdb
        ...
        def r_boundary_db():
            ...
            c.execute("""
                CREATE TABLE orders (
                    ...
                    user_nickname VARCHAR, order_time TIMESTAMP, pay_time TIMESTAMP,
                    ship_time TIMESTAMP, order_type VARCHAR, order_status VARCHAR,
                    product_id VARCHAR, merchant_code VARCHAR, product_title VARCHAR,
                    sku_id VARCHAR, sku_code VARCHAR, sku_name VARCHAR,
                    quantity INTEGER, amount DOUBLE, refund_status VARCHAR,
                    refund_amount DOUBLE, actual_amount DOUBLE, province VARCHAR,
                    city VARCHAR, influencer_name VARCHAR, influencer_id VARCHAR,
                    live_room_id VARCHAR, video_id VARCHAR, traffic_source VARCHAR,
                    traffic_type VARCHAR, seller_note VARCHAR, year INTEGER,
                    month INTEGER, is_member BOOLEAN, spu_category VARCHAR,
                    spu_type VARCHAR, spu_tier VARCHAR, spu_product_class VARCHAR,
                    spu_product_subclass VARCHAR, spu_cosmetic VARCHAR,
                    spu_spec VARCHAR, channel VARCHAR, is_goujinjin BOOLEAN,
                    is_refund BOOLEAN
                )
            """)
            # activity_start = 2025-06-01
            # UA: 2025-05-22 (recency=10天, 近1个月), F=2
            # UB: 2025-04-12 (recency=50天, 近2-3个月), F=1
            # UC: 2025-01-01 (recency=151天, 近4-6月), F=1
            for oid, uid, t in [
                ("O1","UA","2025-05-28 10:00"), ("O2","UA","2025-05-22 10:00"),
                ("O3","UB","2025-04-12 10:00"),
                ("O4","UC","2025-01-01 10:00"),
            ]:
                c.execute(f"""INSERT INTO orders VALUES
                    ('{oid}','{oid}-1','{uid}','用户','{t}','{t}',NULL,'普通','交易成功',
                     NULL,NULL,'产品','SKU','SC','名称',1,100,NULL,0,100,'浙江','杭州',
                     NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,2025,1,TRUE,'护肤','精华','高端','功效类',
                     '美白','30ml','货架',FALSE,FALSE)""")
            ...
            """)
            c.execute("INSERT INTO user_first_purchase VALUES ('UA', '2023-01-01')")
            c.execute("INSERT INTO user_first_purchase VALUES ('UB', '2023-01-01')")
            c.execute("INSERT INTO user_first_purchase VALUES ('UC', '2023-01-01')")
            return c

        from backend.services.breakdown_service._shared import _get_r_interval_current_distribution

        conn = r_boundary_db()
        result = _get_r_interval_current_distribution(conn, activity_start="2025-06-01")
        conn.close()

        # 构建 r_interval → f_segment → user_count 的映射
        bucket = {}
        for row in result:
            key = (row["r_interval"], row["f_segment"])
            bucket[key] = row["user_count"]

        # UA: recency=10天 → 近1个月, F=2 → F>1
        assert bucket.get(("近1个月已购客", "F>1"), 0) >= 1
        # UB: recency=50天 → 近2-3个月, F=1
        assert bucket.get(("近2-3个月已购客", "F=1"), 0) >= 1
        # UC: recency=151天 → 近4-6月, F=1
        assert bucket.get(("近4-6月已购客", "F=1"), 0) >= 1

    def test_r_interval_date_validation_rejects_bad_format(self):
        """_r_interval_sql 应拒绝非 YYYY-MM-DD 格式"""
        from backend.services.breakdown_service._shared import _r_interval_sql

        with pytest.raises(ValueError, match="YYYY-MM-DD"):
            _r_interval_sql("col", "2025/06/01")

        with pytest.raises(ValueError, match="YYYY-MM-DD"):
            _r_interval_sql("col", "06-01-2025")

        with pytest.raises(ValueError, match="YYYY-MM-DD"):
            _r_interval_sql("col", "invalid")

    def test_r_interval_date_validation_accepts_valid_format(self):
        """_r_interval_sql 应接受合法 YYYY-MM-DD 格式"""
        from backend.services.breakdown_service._shared import _r_interval_sql

        result = _r_interval_sql("col", "2025-06-01")
