// Compiler-only cockpit HTTP contract; plugin view stays NOT_CONNECTED.
import type { components, operations } from '../src/cockpit-contract.generated.js';

type Created = components['schemas']['AnalyticsCockpitCreateRequest'];
type Add = components['schemas']['AnalyticsCockpitAddOp'];
type Saved = components['schemas']['AnalyticsDashboard'];
type Listed = components['schemas']['AnalyticsDashboardList'];
type CreateHeaders = NonNullable<operations['analytics_dashboard_create']['parameters']['header']>;

export const createBody: Created = { title: '我的驾驶舱' };
...
export function createSuccessStatus(status: 200 | 201): keyof operations['analytics_dashboard_create']['responses'] {
  return status;
}
