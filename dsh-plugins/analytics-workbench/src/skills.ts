...
export function apply(ctx: Context): void {
  ...
  const pack = queryMode ? queryPack : b0Pack;
  const skillName = pack.skillName;
  const resourceTool = pack.resourceToolName;
  const stateKind = queryMode ? 'analytics-query-state' : 'analytics-b0-state';
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      schema: { type: 'object', additionalProperties: false, properties: {
        schema_version: { type: 'string', const: queryMode ? 'analytics-channel-followup-skill-resource/v1' : 'analytics-b0-skill-resource/v1', required: true },
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    if (!resource) return { kind: 'deny', reason: queryMode ? 'Unregistered query method resource' : 'Unregistered B0 method resource' };
    ...
    try {
      ...
      return decision;
    } catch { return { kind: 'deny', reason: queryMode ? 'Current query run permission or budget refused method read' : 'Current B0 run permission or budget refused method read' }; }
  });
  ctx.tools.guard(exec => methodTool(exec.name) && granted.get(exec.token) !== identity(exec)
    ? (queryMode ? 'No current query method-read grant' : 'No current B0 method-read grant') : undefined);
