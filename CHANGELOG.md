...
- 侧栏登录/主题：收起后主题钮叠在 rail；未登录显示「未登录」，本机写入显示名称后显示名字/飞书；登录菜单更不透明。rail 选择器改 `:has(.sm-login[data-wide="0"])`。不改 DSH 上游。不上公网。
- 侧栏灰玻璃、主聊天纯白；发送/停止统一品牌色；生成驾驶舱在 composer.dock。不改 DSH 上游。不上公网。
- DSH 钉升到 **0.1.6-alpha.2**（`ddefc45f`）。alpha.1 上游备份保留。合入不等于公网发布。不上公网。
- 账本：STATUS origin/main 对齐 `#206` `f766e5d2`。合入后指针会再差本 PR 一拍。不上公网。
