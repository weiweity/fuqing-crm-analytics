...
- **`scripts/etl/ingest.py:_clean_processed_updates()`** (新增) — Step 4.5 加载成功后清理 entry: **置 `cold_start_marked=False` (保留字段, 不删)** + 追加 `last_processed_at` 时间戳. ⚠️ 字段必须保留 (否则被 `_file_changed` ① 误判为老格式, 每天增量全量重载, 死循环).

### Added
- **`backend/tests/test_coldstart_false_positive.py`** — 9 个测试覆盖: 冷启动强制重载 / 老格式迁移 (Option B 核心) / 加载成功后停止重载 (防死循环) / mtime+hash 短路 / 边界 / 幂等性.
...
### Fixed
- **`backend/tests/test_sim_prod_etl.py`** — RSS 上限 6144→10240 MB. 测试套件增长 (516 tests) 致累积 RSS 超 6GB 阈值.
- **`backend/tests/test_w3w4_pipeline_smoke.py`** — `w3w4_smoke_env` fixture 补 DUCKDB_PATH patch (preload_rfm/backend.db.init/backend.db.connection 各自缓存 import 引用). 修 `create_user_rfm_table` stub 路径 `backend.database`→`backend.db.init`.
...
### Verified
- 删除 `processed_files_shop.json` / `processed_files_member.json` 中 6/12-6/14 被误标的条目后，跑一次 `--update` 成功恢复：
  - 6/12: 4,448 orders
  - 6/13: 8,575 orders
- `pytest backend/tests/test_w3w4_pipeline_smoke.py backend/tests/test_w6_pipeline_integration.py backend/tests/test_w6_etl_notify.py` — 27 passed, 0 failed
