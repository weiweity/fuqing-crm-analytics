...
| 4 | **版本状态** | v0.4.14.32（main @ 64ab54a + Sprint 189 — L4.35 skill symlink verify 治理修 100→0 false positive: 只对双端都有的 skill 校验 (skip workbuddy-only / claude-only), 验证脚本 `1 OK / 0 drift (skip 100 workbuddy-only / claude-only)`. 真因: WorkBuddy 端 107 skill / Claude Code 端 81 skill, 仅 1 双端共有 (ad-hoc-query) — 其他 106 是 WorkBuddy 生态独占 (brainstorming/pdf/xlsx/amazon 等), 跟 L4.35 SSOT 无关. scripts/session_start_check.py:_verify_skill_symlinks 升级 docstring + 加跳过逻辑. pytest 不退化, ruff 0 errors. 累计 118 sprint 0 debt 持续 (Sprint 189 纯治理, 0 业务代码改动, 跟 Sprint 89 / 167 模式 stable), L4.x 36 stable 0 追加, fix_pattern #73 持续, /document-release 累计 20 次. |
