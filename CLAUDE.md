...
| 4 | **版本状态** | v0.4.14.30（main @ 4be74bc + Sprint 187 — CI #500 / #499 / #497 累计 3 sprint CI 复发 root 因治根 + L4.41 subprocess 注入 env[PYTHONPATH] 必须 str(PROJECT_ROOT) 绝对路径 不 inherit 父进程 永久规则 + fix_pattern #72 (Sprint 182 L4.32 macOS 假设被 Linux GitHub Actions runner 反噬: PYTHONPATH=. literal → 子 Python 找不到 backend.services) + mcp_servers/fuqing_adhoc/server.py:_run_cli 修 `_PYTHONPATH = _CWD` (不再 os.environ.get) + TestRunCliSubprocess 4 case regression (macOS 4 pass + Linux 模拟 4 pass `PYTHONPATH=.`). pytest 893/73 skip 持续, ruff 0 errors, 累计 116→117 sprint 0 debt (Sprint 187 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +10 sprint), L4.x 34→**35 stable** (新增 L4.41), fix_pattern #72 沉淀, /document-release 累计 18 次. |
