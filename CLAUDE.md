...
| 4 | **版本状态** | v0.4.14.28（main @ 00fbdfe + Sprint 185 + 1 squash Sprint 185 — CI 跨 3 sprint 复发真因治根 + L4.39 macOS-only test skipif 永久规则 + L4.40 post-merge 自动 branch_cleanup 永久规则 + 拆 TestSprint183L4CrossPlatform 跨平台 class + .githooks/post-merge 加 scripts/branch_cleanup.py 自动跑 (失败不阻 merge). pytest 893/73 skip 持续 (本地), CI Linux 期望 sprint 185+ 后 4/4 jobs 全绿, ruff 0 errors, 累计 114→115 sprint 0 debt (Sprint 185 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +8 sprint), L4.x 32→**34 stable** (新增 L4.39 + L4.40), 11 hook 闭环 (7 Claude Code + 4 git hooks, + post-merge 自动 branch_cleanup 配套 L4.31), git remote SSH 推送 0 timeout, fix_pattern #71 沉淀 (post-merge zombie 漏删 → pre-push hook pytest fail 5 分钟卡点), CI #28529340265 + #28525655029 + #28525438510 跨 3 sprint 复发 100% 治本. |
