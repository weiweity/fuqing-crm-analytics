...
| 4 | **版本状态** | v0.4.14.39（main @ `215c763` + Sprint 203 R2 amend: **3 P1 真 bug 治本** — Finding 2.2 dual_conn Semaphore + Finding 4.1 ClickHouse POC 启动条件监控 launchd weekly + Finding 4.6 /metrics dashboard OpsView.vue, 跟 L4.14 amend 1:1 stable 0 业务代码改动模式. 改动 8 件: ① `backend/services/dual_conn.py` 加 `threading.Semaphore(READ_POOL_SIZE * 2)` 防 burst 下 DuckDB 连接无界增长 (5+ 业务分析师并发取数场景, 跟 L4.10 平台守卫 + L4.40 fail-open 1:1 stable) ② `scripts/clickhouse_poc_monitor.py` ClickHouse POC 启动条件监控 (DuckDB > 200GB / P95 > 30s / 5+ 业务分析师并发 3 件 a/b/c 启动条件, 当前 117.4GB < 200GB → 0 触发 → PASS) ③ `scripts/launchd/com.fuqing.clickhouse-poc-monitor.weekly.plist` launchd weekly 周日 04:45 跑 (跟 R6/R7/R8 04:00/04:15/04:30 错开, 跟 L4.7 launchd 首选 python3 1:1 stable) ④ `backend/tests/test_clickhouse_poc_monitor.py` 5 case / 5 TestClass 锁回归 (跟 L4.59 R6/R7/R8 pytest 1:1 stable + L4.61 跨 CI runner fail-open) ⑤ `frontend-vue3/src/views/OpsView.vue` + `/ops` route 显示 `/metrics` Prometheus 文本解析后 query 计数 + P50/P95/P99 延迟 (跟 L4.52 observability 1:1 stable, 30s poll) ⑥ `docs/sprints/SPRINT203_ARCHITECTURE_REVIEW.md` 375 行架构审查作为 L4.42 立项实证输入 ⑦ `L4.62 launchd plist XML 注释规则` 永久规则化 (plutil -lint OK 才算合规, 跟 db-size-alert/pre-existing-fail-monitor/adhoc-hitrate-monitor/memory-size-monitor 4 个现有 plist 1:1 stable) ⑧ 1 amend commit `215c763` 简化 plist 注释让 plutil OK. 7 files + plist 修复 1 amend commit + docs 改动 = 累计 8 files / +895/-19 across 2 commits (`9f72b23` + `215c763`). 0 业务代码改动模式 stable (跟 Sprint 60+ 累计 30 次 1:1 stable, 跟 Sprint 200 R1 v2.1 1:1 stable). pytest focused 5/5 + 3/3 R6 cross-stable PASS, ruff scoped All checks passed, vue-tsc exit=0, npm build 1.47s OpsView bundled. Live verify: uvicorn PID 24996 + vite preview PID 27466 restart 后 4 端点全 200 (`/api/v1/health` + `/metrics` + `/` + `/ops`), launchd `com.fuqing.clickhouse-poc-monitor.weekly` RunAtLoad 触发 4 次 `CLICKHOUSE_POC_MONITOR_PASS`. 累计 Sprint 60+ 0 debt stable **134 sprint** (跨 +30 sprint), /document-release 真治本累计 **39 次**. L4.x 61 → **62 stable** (新增 L4.62 launchd plist XML 注释规则, 跟 L4.7/L4.40/L4.59/L4.60/L4.61 永久规则配套). | — Sprint 203 architecture review 实证 3 P1 真 bug, 跟 L4.14 amend 1:1 stable 1 commit + 1 plist 修复 amend 收口. 后续 Sprint 203 R3 立项 OpsView STUB TODO 接入 (DuckDB size + manifest version + read pool 利用率) + Sprint 203+ ClickHouse POC 启动条件 b/c 接入. |
