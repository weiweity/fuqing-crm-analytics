...
| L3 (已闭环) | ~~弃 `{valid_sql}` 字符串内嵌~~ → **Sprint 53.5 治本**: `churn.py` 5 处 `{valid_sql}` + channel/level/granularity/category_id f-string 内嵌全部走 `FilterBuilder.build()`. 新增 service 函数强制用 FilterBuilder, 禁止 f-string 内嵌用户输入 | review skill 强制 | **Sprint 53.5** | 本节 |
