...
| 4 | **版本状态** | v0.4.14.157（main @ d7f0f6f amend，2026-06-24 Sprint 104 收口 + Step 12.6 amend 闭环 L4.22，pytest baseline 819/23/0 持续，累计 54 sprint 0 debt，L4.x 22 条 stable，VERSION 不 bump） |
...
| **改 contract 字段** | 增删改 `backend/contracts/*.py` 字段 (类型/范围/命名) | 跑 `python -m backend.contracts._lint` | 未跑 lint → 禁止 commit |
...
| 层 | 防御 | 触发点 | Sprint | 文件 |
|---|---|---|---|---|
| L1-L3 (历史) | ~~Sprint 33-54 跨 sprint AI write safety net (frontend .vue + backend SQL f-string + e2e spec-lint + FilterBuilder)~~ | Sprint 130 P0-1 + P0-3 + P0-4 删 devOps toolchain + e2e + pytest 后失效 (跟 lint 钩子 / pytest fixture 相关) | Sprint 33-54 | 历史归档 |
...
| **L4.9-L4.10 (历史)** | ~~GH Action tag 验证 + 平台检查 main~~ | Sprint 130 P0-1 删除 devOps toolchain 后 L4.9 失效 (跟 GH Action 升级相关); L4.10 仍有效 (跟 code 架构相关) | — | — |
...
| **L4.16-L4.18 (历史)** | ~~devOps 工具链相关~~ | Sprint 130 P0-1 删除 devOps toolchain 后失效 (跟 GH Actions / Node 升级 / ruff-action 相关) | — | — |
| **L4.16 (流程, Sprint 130 失效)** | _devOps 删除后失效, 历史规则沉淀_ | — | **Sprint 82/83** | 历史归档 |
...
| **L4.21-L4.22 (历史)** | ~~真连 CI 验证 + vite preview rebuild dist~~ | Sprint 130 P0-1 删除 devOps toolchain + 后续 P0-3 删 e2e 后失效 (跟 CI shallow clone / vite dist rebuild 相关) | — | — |
