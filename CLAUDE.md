...
| **L5.2 (流程)** | **spec 写法"环境无关"原则**: ① 不 hardcode 业务数据长度(`toBe(5)` 禁, 用 `length > 0` 替); ② 不 `waitForTimeout(N)` 死等(用 `waitForSelector` / `expect.toBeVisible` 替); ③ `page.request` 加 Authorization header(从 sessionStorage 拿 `fq_crm_auth_token`). 配合 `frontend-vue3/e2e/lint/spec-lint.sh` pre-commit hook(起步 advisory, 1-2 sprint 观察 false positive 率后改 blocking). 防 Sprint 41.5/41.6/41.8/41.9 类实战 fix 复发 | review skill 强制 | **Sprint 42** | 本节 |
