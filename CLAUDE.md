...
| 4 | **版本状态** | v0.4.14.34（main @ afa7459 + Sprint 191 — MCP stdio 协议 LSP → newline JSON 重写. WorkBuddy 子 agent 反馈包触发: `mcp_servers/fuqing_adhoc/server.py` 原 LSP Content-Length framing 导致 WorkBuddy `MCP error -32001: Request timed out` 120s. 真因: MCP stdio 标准 = newline-delimited JSON (`read: line = sys.stdin.buffer.readline(); json.loads(line)` + `write: json.dumps(...) + b"\n"; flush()`), **不是** LSP `Content-Length: N\r\n\r\n` + body. LSP 实现的 server 在 `_read_message()` 永久阻塞. 诊断教训: 用 server 期望的 LSP 格式测 server = 假阳性陷阱, 永远用客户端实际发送的协议格式测. L4.44 永久规则 stable: MCP stdio 协议必须 newline JSON, 禁照搬 LSP. 配套工具 `~/.workbuddy/skills/mcp-stdio-protocol-debugging/references/diagnose_mcp.py` 一键对比测试. 32 pytest 零回归 (含 `test_content_length_upper_bound_prevents_dos` 1MB 限制保留). 累计 118 sprint 0 debt 持续 (跨 Sprint 60+ 0 debt stable 模式 +12 sprint), L4.x 35→**36 stable** (新增 L4.44), fix_pattern #75 沉淀 (MCP stdio 协议混淆), /document-release 累计 22 次. |
