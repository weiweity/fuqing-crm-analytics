...
| 4 | **版本状态** | v0.4.14.31（main @ 9b7b7f9 + Sprint 188 — 全部 backlog 处理 sprint (B1+B2+B4 工程 + B3 反漂移教训 + B5+B6 健康度) + 物理 rm 3 个空 feature branch + L4.42 立项信息必须 git log / grep 实证禁止凭印象 永久规则 + fix_pattern #73 (Sprint 184/187 close memory "25 处 os.chdir" 凭印象漂移 → Sprint 188 B3 Codex 严格 git 实证 + grep `backend/tests/` 0 处真实风险撤销立项) + backend/routers/ad_hoc_query.py (Sprint 188 B1 HTTP API 9 endpoint +11 test) + mcp_servers/fuqing_adhoc/server.py 真端到端 7 test (Sprint 188 B2 WorkBuddy GUI) + scripts/ci_cross_sprint_drift.py (Sprint 188 B4 跨 sprint 隐式 fail 检测 10/10 commit PASS 0 drift) + pytest 844 passed / 85 skip, ruff 0 errors, 累计 117→118 sprint 0 debt (Sprint 188 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +11 sprint), L4.x 35→**36 stable** (新增 L4.42), fix_pattern #73 沉淀, /document-release 累计 19 次. |
