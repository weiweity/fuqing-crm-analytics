...
| 4 | **版本状态** | v0.4.14.26（main @ 3023a07 Sprint 183 — WorkBuddy /ad-hoc-query 优化 + SKILL.md v2.2 (顶部 "执行路径强制" + "需求-工具映射速查表" + "锁冲突 graceful fallback" 3 段) + L4.36 禁停 uvicorn 永久规则 + daily-gsv-multi-period 第 11 个 MCP tool 新子命令 (多周期 × 8 维度) + QA 抓到的 3 个真 bug 治本 (双 registry 注册丢失 / query name hyphen 化 / 4 unused imports) + 删 2 个临时脚本 (adhoc_daily_segments + adhoc_transpose) + 5 个端到端锁回归 test, pytest baseline 78 (含 Sprint 183 +9 cases + 5 端到端锁回归), ruff 0 errors, 累计 113 sprint 0 debt (Sprint 183 全部治本, 跨 Sprint 60+ 0 debt stable 模式 +6 sprint), L4.x **30 stable** (新增 L4.36), 11 hook 闭环 (7 Claude Code + 4 git hooks), git remote SSH 推送 0 timeout, fix_pattern #68-#69 沉淀 2 模式 (pytest collection 自动 import 掩盖 registry bug + argparse subcommand name 严格匹配) |
