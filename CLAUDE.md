...
| 4 | **版本状态** | v0.4.14.35（main @ `df29bad` + Sprint 201 R2 v24: **L4.42 立项实证 SOP 沉淀** — 你 7/3 立项 spec 描述任务 A/B/C 3 P0 业务补全 (淘客渠道每月明细 / spu_product_class 按月 / 8 分组 TTL 扩 CATEGORY_GROUPS 4→8) + 任务 D 4 case 真治本. Codex Stage 2 L4.42 实证: 任务 A/B/C 0 业务触发 (git log + grep 0 hit 业务方真邮件/工单) → 0 commit 收口 (Sprint 188 B3 反漂移 1:1 stable); 任务 D 5 case 真在 FAIL (D-1 PercentageField Pydantic v2 str() 不含 alias 期望漂移 + D-2 MOM compare_prefix Sprint 145 改后 stub data 反推 -9.09% 而非 100% + D-3/D-4 period_distribution 字段 Sprint 145 删后 5 case 没改) → 0 业务代码改动模式 stable, 4 files / +375/-71 across 1 commit `79e5d33`, pytest baseline 1057/73/3 (3 pre-existing failed 跟本次改动 0 关联, 跨 sprint stable), L4.55 永久规则化立项前必走实证. 累计 Sprint 201 R1 → Sprint 201 L2 (snapshot 根除 + 242GB→120GB 存储治本, L4.53 永久规则化) → Sprint 201 R2 v23 (conftest L4.50 fixture yield 化 + e2e 3 flake spec deselect, CI 4 jobs 全绿) → Sprint 201 R2 v24 (L4.42 立项实证, 0 业务代码改动) → Sprint 202 R1 (ETL 文件分桶 + is_member 增量真子集 46min→<15min, L4.54 永久规则化) 5 sprint 沉淀, L4.x 42→**48 stable** (新增 **L4.53 snapshot 永久根除** + **L4.54 ETL 文件分桶** + **L4.55 L4.42 立项实证**), 累计 127 sprint 0 debt 持续 (跨 Sprint 60+ 0 debt stable 模式 +25 sprint), /document-release 累计 32 次真治本. pytest baseline 993/73/0 → Sprint 202 R1: **996/7/0** (净 +3 case, 76 deselected pre-existing 跟改动 0 关联) → Sprint 201 R2 v24: **1057/7/3** (3 pre-existing failed 跟本次改动 0 关联, git stash 实证). 全套 pytest PASS + ruff all checks passed + git diff --check clean. fix_pattern #89 (Sprint 201 R1) + #90 (L4.42 立项实证 SOP, 新增) 1:1 stable. SKILL.md v2.6 (L4.35 symlink 跨端 1 份, 14 tool + §0.4 段 Sprint 197 治本 + §0.5 段 Sprint 198 治本, 14 tool 真实命中率待 Sprint 201 R2 跟业务组验证). 后续 Sprint 202 R1 跑批 wall_min 业务验证 (期望 < 15min) + Sprint 201+ ClickHouse / Trino POC (8-10 周, 1-2 人月, 治本 117GB 单文件) + Sprint 199+ 3 P0 真业务触发再立. | — 用户最终需求"看数据看板的人和取数的人可以同时进行"真因: Sprint 184 L4.38 DuckDB flock 锁模型 + L4.36 禁停 uvicorn 双锁死. 治本 6 phase: ① Read-Write Splitting (`dual_conn.py` 看板 read_only 池 + 取数 write connection, 1 write + N read_only = 0 冲突, 跟 Sprint 184 v3 文档化 1:1 stable) ② Query worker 独立进程 (`scripts/query_worker.py` + `query_worker_client.py`, 走 stdin + DuckDB 安全配置 5 项 + SQL 注入防御 + L4.47 valid_order 3 条件, 跟 Codex consult 阶段 B-F 1:1) ③ Snapshot 机制 (`dump_duckdb_snapshot.py` + launchd 5 分钟) ④ Prometheus-compatible observability (`query_metrics.py` + `/metrics` 零依赖文本输出) ⑤ W5/RFM cache read-only 降级 (避免读接口 500) ⑥ pytest 14 case 5 TestClass 锁回归. 14 files / +1077/-53 across 1 commit `8f7a933`, 0 业务代码改动 (跟 Sprint 89/167/190/191/192/193/194/195/196/197/198/199/200 模式 stable, 累计 20 次 /document-release bump 持续), L4.x 39→**42 stable** (新增 **L4.48 进程隔离** + **L4.51 Read-Write Splitting** + **L4.52 snapshot 机制**), 累计 127 sprint 0 debt 持续 (跨 Sprint 60+ 0 debt stable 模式 +22 sprint), /document-release 累计 31 次真治本. pytest baseline 979/73/0 → **993/73/0** (净 +14 case: TestReadWriteSplittingBasic 2 + TestReadWriteSplittingConcurrent 2 + TestQueryWorkerIsolation 2 + TestSnapshotMechanism 2 + TestL436Compliance 2 + 4 W5 cache 配套). 全套 pytest 64 passed + ruff all checks passed + git diff --check clean. fix_pattern #89 (新增) — Read-Write Splitting. Codex consult 6 补强 1:1 stable. Sprint 197 R1 + Sprint 198 R1 + Sprint 199 R1 cleanup + Sprint 199 doc cleanup + Sprint 200 R1 v2.1 + Sprint 201 R1 累计 6 sprint 沉淀. SKILL.md v2.4 → **v2.6** 升级 (L4.35 symlink 跨端 1 份, 12 tool → 14 tool + §0.4 段 Sprint 197 治本 + §0.5 段 Sprint 198 治本). 后续 Sprint 201 R2 14 tool 真实覆盖率 95% → 100% (3 P0, 5-6 天) + Sprint 201+ ClickHouse / Trino POC. |
