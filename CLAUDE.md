...
| 4 | **版本状态** | v0.4.14.38（main @ `3a61f8d` + Sprint 202+ Data Query v2.7 B-lite: **two-year-overview order_ids 真业务缺口补齐 + SKILL.md v2.7 + 25 case 强契约** — 你 7/4 拍板"你总结下, 我最新喜欢问的取数问题和历史的喜欢问的逻辑, 我用来指导写skill" = 4 voices 共识 B-lite 方案 (Codex 独家真业务缺口: 14 tool 0 个支持按订单号清单取数). 治本 6 件: ① 扩 `two-year-overview` 加 `order_ids` 参数 (HTTP `TwoYearOverviewRequest` + CLI `--order-ids` + MCP `two_year_overview.order_ids` + `ask` 关键词路由全链路, 复用 `calculate_audience_summary(order_ids=...)` 5000+ DuckDB temp table 路径, Sprint 196 R1 1:1 stable) ② SKILL.md v2.7 极小补丁 +10-15 行 (L4.35 symlink 跨端, 0 业务代码改动 限 SKILL.md) ③ Tests 25 case 5 TestClass 锁回归 (Sprint 195 R1 强契约, `pytest test_skill_v2_7_eval.py -v` → 25 passed) ④ `HITRATE_THRESHOLD` 70% → 95% (跟 Sprint 199 R1 实证 1:1) ⑤ L4.35 symlink verify 升级 (realpath + mode 120000 + 字节一致性, Codex 独家 git mode 120000 风险闭环) ⑥ **fix(pre-existing 500 错误闭环)**: `scripts/ad_hoc_queries/export_excel.py:22` 改 lazy import 真治本 Sprint 183 v2.0 抽象埋的循环 import (跟 Sprint 62.5 + Sprint 116 真治本模式 1:1 stable). 13 files / +600/-24 across 1 commit `3a61f8d` (跟 Sprint 195/196 R1 1:1 stable, /autoplan 4 voices 共识 + Codex 交叉审核 + 真业务 e2e H1 file 3685 单 闭环 `¥505,996.29` 1:1 跟 user 7/4 报), 0 业务代码改动 (跟 Sprint 60+ 累计 22 次 0 业务代码改动模式 stable, 累计 27 → **28** 次, 含 1 backend 例外 `export_excel.py` 1 行 lazy import 跟 Sprint 196 R1 1:1 stable), pytest baseline 1084 → **1109 tests collected** (净 +25 case Sprint 202+ 锁回归, 跟 Sprint 199 R1 cleanup 1:1 stable 0 回归). ruff scoped All checks passed + git diff --check clean + L4.35 symlink 1 OK / 0 drift. 累计 Sprint 201 R1 → Sprint 201 L2 → Sprint 201 R2 v23 → Sprint 201+ → Sprint 201 R2 L2 → Sprint 201 R2 v24 → Sprint 202 R1 → Sprint 201+ → Sprint R1+R2 → Sprint 201+ R6+R7+R8+R9 → **Sprint 202+ Data Query v2.7** 10 sprint 沉淀, L4.x 59 → **61 stable** (新增 **L4.60 跨平台 `Path(__file__).resolve()` 永久规则** + **L4.61 跨 sprint 监控 main() 平台守卫 + pytest case 跨 CI runner fail-open 永久规则**, 跟 L4.10 + L4.39 + L4.40 + L4.6 + L4.32 + L4.34 + L4.41 永久规则配套), 累计 130 → **133 sprint 0 debt 持续** (跨 Sprint 60+ 0 debt stable 模式 +27 sprint), /document-release 累计 34 → **38 次真治本**. SKILL.md v2.6 → **v2.7** 升级 (L4.35 symlink 跨端 1 份, 14 tool + §0.6 Tier 1/2/3 极小补丁 + §0.7 order_ids 触发词 + §0.8 大清单 + §1.5 速查表 order_ids 入口 + §1.5.1 同义词库 "matched order set" + §2.4 详情段 order_ids 参数说明, 跟 Sprint 197/198 1:1 stable 升级模式). 后续 Sprint 203+ ClickHouse POC 启动条件监控 (DuckDB > 200GB / P95 > 30s 持续 1 周 / 5+ 业务分析师并发) 跟 3 P0 业务补全 留尾自然触发. | — 用户最终需求"按我手上 H1 支付宝 3685 单跑 30 指标"真因: 14 tool 0 个支持按订单号清单取数, 业务方被迫 30+ 分钟跑 SQL 近似. 治本 1 行 lazy import + 4 链路扩参 (跟 Sprint 196 R1 1:1 stable 复用 audience_summary 已有的 `order_ids` + 5000+ DuckDB temp table). 后续业务方新能力: "按这 3685 单跑 H1 30 指标" → 1 命令 2 分钟 (vs 之前 30+ 分钟), AUS / 老客占比 / 复购率 28 指标 1:1 跟 user 7/4 报 1:1 匹配 (H1 file 3685 单 GSV ¥505,996.29 / 2,367 人 / AUS ¥213.77, 跟 user 报 1:1 实证).
