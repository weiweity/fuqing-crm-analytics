...
# Sprint 31.1: SQLite sidecar tracker (5 次复发终极治根, 替代 prefix matching
# 机制错误). tracker 不可用时降级到原 FQ_TMP_PREFIXES glob 路径
from scripts.etl.common.tmp_tracker import TrackerDB, TRACKER_DB_PATH
...
# Sprint 31.1: FQ_TMP_PREFIXES 现为 legacy / bootstrap-only. Source of truth
# 改为 TrackerDB (TRACKER_DB_PATH), tracker 不可用时降级到 glob 路径.
...
_FQ_TMP_TRACKER_PATH = TRACKER_DB_PATH  # Sprint 31.1: tracker DB 路径 (test 可 monkeypatch)


def _collect_fq_tmp_orphans() -> list:
    """Sprint 31.1: tracker 是 source of truth, prefix 是 fallback.

    设计:
      1. tracker.bootstrap_from_filesystem(FQ_TMP_PREFIXES) 拾起外部副本
         (workbuddy cache / subagent copy, 103GB fuqing_sampling2.duckdb 模式)
      2. tracker.list_expired(24) 拿候选 (create_at = mtime, 24h+ 阈值)
      3. tracker 不可用 / 出错 → 降级到原 FQ_TMP_PREFIXES glob 路径
      4. F7 symlink 跳过 (跟原一致)
      5. 按 mtime 倒序 (最老优先, 同 mtime 按 size 倒序)

    Returns:
        list of (path, size_bytes, age_h) tuples, oldest first.
    """
    candidates = []
    tracker = TrackerDB(db_path=_FQ_TMP_TRACKER_PATH)

    if tracker.is_available():
        try:
            tracker.bootstrap_from_filesystem(prefixes=FQ_TMP_PREFIXES)
            for path, size_bytes, age_h in tracker.list_expired(age_hours=_FQ_TMP_MIN_AGE_HOURS):
                # F7 修复：symlink 跳过 (跟原一致, tracker 自身 bootstrap 也跳)
                if os.path.islink(path) is True:
                    _safe_log(f"  [tmp-cleanup] skip symlink: {path}")
                    continue
                candidates.append((path, size_bytes, age_h))
        except (OSError, sqlite3.Error) as e:
            # 软失败: tracker 任何错误降级到 glob 路径
            _safe_log(f"  [tmp-tracker] error, fallback to glob: {e}")
            candidates = []

    if not candidates:
        # Fallback: 原 FQ_TMP_PREFIXES glob 路径 (跟 Phase 1 之前完全一致).
        # Sprint 25 F5 first-prefix starvation 修复: 全量收集 + 排序 + 截断.
        now = time.time()
...
def _cleanup_fq_tmp_orphans() -> int:
    ...
    # Sprint 31.1: tracker 用来在 os.remove 后同步 remove row (保持 source of truth 一致)
    tracker = TrackerDB(db_path=_FQ_TMP_TRACKER_PATH)
    ...
    for path, size_bytes, age_h in candidates:
        ...
        try:
            ...
            # Sprint 31.1: os.remove 成功后同步 remove tracker row
            # 软失败 (tracker.remove 内部已包 try/except), 不阻塞 cleanup
            tracker.remove(path)
        except FileNotFoundError:
            # Sprint 31.1: file 已被外部删除 (test 隔离 / 手工 rm).
            # 同步 remove tracker row, 不计入 deleted (我们没做工作).
            _safe_log(f"  [tmp-tracker] stale row, file already gone: {path}")
            tracker.remove(path)
        except OSError as e:
