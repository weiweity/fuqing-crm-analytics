#!/bin/bash
# Sprint 20+ A' 路径一键激活: DuckDB 1.6.0 stable 升 prod 路径
# 调用前提:
#   1. /tmp/duckdb-1.6.0-stable-available.flag 存在 (PyPI 已发 1.6.0 stable)
#   2. 当前已装 1.6.0.dev12+ (Sprint 20+ A' 路径验证, 或保持现有 1.5.4.dev18 不变)
#   3. v2 code 在 fix/sprint16-p0-duckdb-taoke-channel-race branch
#
# 4 步激活 (跟 A 路径一致, 只换 DuckDB 版本):
#   Step 1: 装 1.6.0 stable (--upgrade + 保留 dev 包)
#   Step 2: 跑 v2 4 unit tests (确认 stable 治根)
#   Step 3: 切到 fix branch + merge --no-ff v2 code 到 main
#   Step 4: 跑 batch 真验 + CHANGELOG v0.4.14.57
#
# 跑法: bash scripts/etl/activate_duckdb_1_6_0_stable.sh
...
LOG="/tmp/fuqing-activate-duckdb-1-6-0.log"

echo "=== DuckDB 1.6.0 stable 激活 (A' 路径) @ $(date) ===" | tee -a "$LOG"
cd "$REPO_ROOT"

# 0. 前置检查
if [ ! -f "$FLAG" ]; then
  echo "[FATAL] $FLAG 不存在, 1.6.0 stable 还没 release (或没跑 check_duckdb_release_cron.sh)" | tee -a "$LOG"
...
# 1. 装 1.6.0 stable
echo "[STEP 1] 装 duckdb 1.6.0 stable..." | tee -a "$LOG"
python3 -m pip install --user --upgrade "duckdb>=1.6.0,<1.7.0" 2>&1 | tee -a "$LOG"
NEW_VERSION=$(python3 -c "import duckdb; print(duckdb.__version__)" 2>/dev/null)
echo "[STEP 1] 装后 DuckDB: $NEW_VERSION" | tee -a "$LOG"

# 验证 stable release (1.6.0.x 不带 .dev)
...
if [ $? -ne 0 ]; then
  echo "[FATAL] v2 tests failed, 1.6.0 stable 不能治根, 中止" | tee -a "$LOG"
  exit 1
fi
...
echo "  5. CHANGELOG v0.4.14.57 + 合 main + /ship" | tee -a "$LOG"
echo "  6. 删 /tmp/duckdb-1.6.0-stable-available.flag" | tee -a "$LOG"
