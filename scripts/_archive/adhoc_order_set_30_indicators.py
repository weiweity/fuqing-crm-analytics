...
用途：
- 用户上传 2026/2025 两份 UDS 订单明细（J 列 = 订单号），分别去重后。
- 按"按年份分别匹配"策略：
  - 2026-05-06 ~ 2026-06-21 窗口：只统计 order_id 在 2026 Excel 订单号集合中的有效订单。
  - 2025-05-06 ~ 2025-06-21 窗口：只统计 order_id 在 2025 Excel 订单号集合中的有效订单。
- 复用 backend.services.metrics.audience_summary 的 _extract_metrics 口径。

输出：
- 标准 30 指标对比表（2026 值、2025 值、YOY、单位）。
...
OUTPUT_SHEET_NAME = "UDS订单集合_30指标"
OUTPUT_TITLE = "UDS订单集合 30指标对比"
