...
assert.ok(['--check', '--write'].includes(mode) && flag === '--python' && isAbsolute(python ?? '') && !extra.length,
  'Usage: competition-computed-contract.mjs --check|--write --python /absolute/python3.14');
...
from backend.contracts.competition_computed import computed_openapi
print(json.dumps(computed_openapi()))
...
const artifacts = new Map([
  ['backend/contracts/analytics-competition-computed.openapi.json', JSON.stringify(schema, null, 2) + '\n'],
  ['dsh-plugins/analytics-workbench/src/competition-computed-contract.generated.d.ts',
...
for (const [relative, content] of artifacts) {
  ...
  else assert.equal(await readFile(join(root, relative), 'utf8'), content, `Computed contract drift: ${relative}`);
}
console.log(`Competition computed runtime contract ${mode.slice(2)} passed`);
