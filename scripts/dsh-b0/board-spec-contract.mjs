/** Offline library canvas type generation. Does not start CRM, SQLite or services. */
...
assert.ok(['--check', '--write'].includes(mode) && flag === '--python' && isAbsolute(python ?? '') && !extra.length,
  'Usage: board-spec-contract.mjs --check|--write --python /absolute/python3.14');
...
from backend.contracts.board_spec import board_spec_openapi
print(json.dumps(board_spec_openapi()))
...
const artifacts = new Map([
  ['backend/contracts/analytics-board-spec.openapi.json', JSON.stringify(schema, null, 2) + '\n'],
  ['dsh-plugins/analytics-workbench/src/board-spec-contract.generated.d.ts',
...
for (const [relative, content] of artifacts) {
  ...
  else assert.equal(await readFile(join(root, relative), 'utf8'), content, `BoardSpec contract drift: ${relative}`);
}
console.log(`Library BoardSpec contract ${mode.slice(2)} passed`);
