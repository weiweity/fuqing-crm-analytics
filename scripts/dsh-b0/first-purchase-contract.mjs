/** Offline first-purchase HTTP contract. Never start CRM/HTTP/DB. */
...
if (!['--write', '--check'].includes(mode) || pythonFlag !== '--python' || !python || !isAbsolute(python) || extra.length) {
  throw new Error('Usage: node scripts/dsh-b0/first-purchase-contract.mjs --check|--write --python /absolute/path/to/python3.14');
}
const generate = String.raw`
import importlib.abc, json, sys
if sys.version_info < (3, 14):
    raise SystemExit('first-purchase contract generation requires explicit Python 3.14+')
class DenyLegacy(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname in {'backend.config', 'backend.routers', 'backend.main', 'backend.db', 'duckdb', 'dotenv'}:
            raise RuntimeError('offline first-purchase generation cannot import legacy services')
...
if schema.get('x-shared-run-kernel') is not True:
    raise SystemExit('first-purchase OpenAPI must declare x-shared-run-kernel')
...
if (result.error || result.status !== 0) {
  throw new Error(`Offline first-purchase schema generation failed: ${result.error?.message ?? result.stderr}`);
}
...
if (!schema.paths?.['/api/v1/analytics-first-purchase/runs']) throw new Error('missing analyses collection path');
if (schema.paths['/api/v1/analytics-query/runs/{run_id}']) throw new Error('first-purchase contract must not publish query session routes');
...
const typed = `/** Generated first-purchase HTTP contract; do not edit. OpenAPI SHA-256: ${digest} */\n${astToString(ast)}`;
const artifacts = new Map([
  ['backend/contracts/analytics-first-purchase.openapi.json', JSON.stringify({ ...schema, 'x-schema-sha256': digest }, null, 2) + '\n'],
  ['dsh-plugins/analytics-workbench/src/first-purchase-contract.generated.d.ts', typed],
]);
for (const [relative, content] of artifacts) {
  const path = join(root, relative);
  if (mode === '--write') await writeFile(path, content);
  else if (await readFile(path, 'utf8') !== content) throw new Error(`first-purchase contract drift: ${relative}`);
}
const program = ts.createProgram([
  join(root, 'dsh-plugins/analytics-workbench/src/first-purchase-contract.generated.d.ts'),
  join(root, 'dsh-plugins/analytics-workbench/test/first-purchase-contract.typecheck.ts'),
], {
...
console.log(`first-purchase contract ${mode === '--write' ? 'generated' : 'verified'}; sha256=${digest}; TypeScript checks passed`);
