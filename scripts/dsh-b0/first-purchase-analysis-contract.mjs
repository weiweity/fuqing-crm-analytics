/** Offline first-purchase saved-analysis HTTP contract. Never start CRM/HTTP/DB. */
...
if (!['--write', '--check'].includes(mode) || pythonFlag !== '--python' || !python || !isAbsolute(python) || extra.length) {
  throw new Error('Usage: node scripts/dsh-b0/first-purchase-analysis-contract.mjs --check|--write --python /absolute/path/to/python3.14');
}
const generate = String.raw`
import importlib.abc, json, sys
if sys.version_info < (3, 14):
    raise SystemExit('first-purchase saved-analysis contract generation requires explicit Python 3.14+')
class DenyLegacy(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname in {'backend.config', 'backend.routers', 'backend.main', 'backend.db', 'duckdb', 'dotenv'}:
            raise RuntimeError('offline first-purchase analysis generation cannot import legacy services')
...
if schema.get('x-first-purchase-saved-analysis-http') is not True:
    raise SystemExit('first-purchase saved-analysis OpenAPI must declare x-first-purchase-saved-analysis-http')
if schema.get('x-shared-succeeded-source') is not True:
    raise SystemExit('shared source must be declared true')
...
if (result.error || result.status !== 0) {
  throw new Error(`Offline first-purchase saved-analysis schema generation failed: ${result.error?.message ?? result.stderr}`);
}
...
if (!schema.paths?.['/api/v1/analytics-first-purchase/analyses']) throw new Error('missing first-purchase analyses collection path');
if (schema.paths['/api/v1/analytics/analyses']) throw new Error('first-purchase analysis contract must not publish channel analysis routes');
if (schema.paths['/api/v1/analytics-query/runs/{run_id}']) throw new Error('first-purchase analysis contract must not publish query session routes');
...
const typed = `/** Generated first-purchase saved-analysis HTTP contract; do not edit. OpenAPI SHA-256: ${digest} */\n${astToString(ast)}`;
const artifacts = new Map([
  ['backend/contracts/analytics-first-purchase-analysis.openapi.json', JSON.stringify({ ...schema, 'x-schema-sha256': digest }, null, 2) + '\n'],
  ['dsh-plugins/analytics-workbench/src/first-purchase-analysis-contract.generated.d.ts', typed],
]);
for (const [relative, content] of artifacts) {
  const path = join(root, relative);
  if (mode === '--write') await writeFile(path, content);
  else if (await readFile(path, 'utf8') !== content) throw new Error(`first-purchase saved-analysis contract drift: ${relative}`);
}
const program = ts.createProgram([
  join(root, 'dsh-plugins/analytics-workbench/src/first-purchase-analysis-contract.generated.d.ts'),
  join(root, 'dsh-plugins/analytics-workbench/test/first-purchase-analysis-contract.typecheck.ts'),
], {
...
console.log(`first-purchase saved-analysis contract ${mode === '--write' ? 'generated' : 'verified'}; sha256=${digest}; TypeScript checks passed`);
