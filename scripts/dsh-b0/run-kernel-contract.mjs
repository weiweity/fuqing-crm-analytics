// Offline B0-only contract generation. Never start the CRM or a network server.
...
if (!['--write', '--check'].includes(mode) || pythonFlag !== '--python' || !python || !isAbsolute(python) || extra.length) {
  throw new Error('Usage: node scripts/dsh-b0/run-kernel-contract.mjs --check|--write --python /absolute/path/to/python3.14');
}
const generate = String.raw`
import importlib.abc, json, sys
if sys.version_info < (3, 14):
    raise SystemExit('B0 contract generation requires explicit Python 3.14+')
class DenyLegacy(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname in {'backend.config', 'backend.routers', 'backend.main', 'backend.db', 'duckdb', 'dotenv'}:
            raise RuntimeError('offline B0 generation cannot import legacy services')
...
print(json.dumps(create_app().openapi(), sort_keys=True, ensure_ascii=False, allow_nan=False))
`;
...
if (result.error || result.status !== 0) {
  throw new Error(`Offline B0 schema generation failed: ${result.error?.message ?? result.stderr}`);
}
...
// Reuse the already installed, version-checked tools. No installs or upgrades.
...
const typed = `/** Generated B0-only contract; do not edit. OpenAPI SHA-256: ${digest} */\n${astToString(ast)}`;
const artifacts = new Map([
  ['backend/contracts/analytics-run.openapi.json', JSON.stringify({ ...schema, 'x-schema-sha256': digest }, null, 2) + '\n'],
  ['dsh-plugins/analytics-workbench/src/run-contract.generated.d.ts', typed],
]);
for (const [relative, content] of artifacts) {
  const path = join(root, relative);
  if (mode === '--write') await writeFile(path, content);
  else if (await readFile(path, 'utf8') !== content) throw new Error(`B0 contract drift: ${relative}`);
}
const program = ts.createProgram([
  join(root, 'dsh-plugins/analytics-workbench/src/run-contract.generated.d.ts'),
  join(root, 'dsh-plugins/analytics-workbench/test/run-contract.typecheck.ts'),
], {
...
console.log(`B0 contract ${mode === '--write' ? 'generated' : 'verified'}; sha256=${digest}; TypeScript checks passed`);
