...
归档 `src.user_rfm` 若没有源窗口末日及更早的 GMV/90 as-of，脚本改为用 `src.orders` 按 `semantic.segments` 阈值现算 GMV/90（as-of `2023-09-15`，回看 90 天，不含购尽进/交易关闭），再 +3 年写到 `SYN26-U-*`。不要把 2026 RFM 抄给合成用户。健康页 RFM 仍走 `rfm_dashboard_full` / `user_rfm_precompute`，本 overlay 不补那两张表。
