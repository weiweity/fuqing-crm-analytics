...
| [CHANGELOG.md](./CHANGELOG.md) | 版本变更记录 (v0.4.14.157, Sprint 101 收口) |
...
当前测试覆盖（**819 passed / 23 skipped**，Sprint 101 全量复验，v0.4.14.157；Sprint 99 baseline 持续；本地 macOS）：
...
| 2026-06-23 | **Sprint 95+96+96.1+96.2+96.3+96.4+96.5 7 sprint 收口 (D2 e2e 50+MB OOM 治本 7 sprint 完整链路全闭环, 跟 Sprint 88+92+92.1 模式 2 sprint 延展, v0.4.14.156 不变, main HEAD 8de3229, 累计 Sprint 56+...+96.5 = 45 sprint 0 debt 持续, L4.x 永久规则 18 条 stable 0 追加, 0 治理 SOP 追加, 7 步实战 fix 模式 = 1) 改 lint.yml 2) 改 e2e.yml 3) 改相关 test 4) 验证 yaml.safe_load 5) pytest 本地 6) commit 7) push + merge + gh run watch, 跳任 1 步 → 必修 2 误诊真因真发现, CI 3/3 jobs ✓ + e2e.yml 独立 workflow 4m26s ✓ success 跟之前 9m35s 比 -5m 跟之前 18m+ 比 -14m)** |
| 2026-06-23 | **Sprint 97 收口 (Sprint 60+ 留尾 #1 FilterBuilder 治标推广 12 service + L4.19 channel alias 永久规则 + /category/flow IndexError 治本, v0.4.14.155 → v0.4.14.156, main HEAD `c3bd320`, 4 commit 0 debt, pytest 811/23/0 + 端到端 12/12 curl 200, 累计 47 sprint 0 debt 持续, L4.x 永久规则 18 → 19 (L4.19 新增))** |
| 2026-06-23 | **Sprint 98 收口 (Sprint 60+ 留尾 #1 FilterBuilder 真治本全闭环 — 改 OrderFilters.channel_in/not_in 加 table_alias 参数 + FilterBuilder 加 self._table_alias + with_table_alias() + 删全 12 service 14 处 .replace() 重复代码 + Codex 主动 audit 补 7 service 单表 SQL orders o 别名, v0.4.14.156 → v0.4.14.157, main HEAD `3cd2d87`, 4 commit 0 debt, pytest 815/23/0 + 端到端 12/12 curl 200, 累计 48 sprint 0 debt 持续, L4.x 永久规则 19 stable, 0 业务代码改动外的越界 + 0 治理 SOP 追加)** |
| 2026-06-23 | **Sprint 99 收口 (留尾治理 sprint 模式 — 留尾 #11 SSOT 漂移闭环 + L4.20 反 SSOT 漂移永久规则根治 + check_ssot_drift.py 结构化 ground-truth-lint + 4 case regression test, v0.4.14.157 不变, main HEAD `e118178`, 1 commit 0 debt, pytest 819/23/0 (815 baseline + 4 case), 累计 49 sprint 0 debt 持续, L4.x 永久规则 19 → 20 (L4.20 新增, 跟 L4.7 + L4.19 配套), 跨文档一致性 100% PASS, Sprint 60+ 留尾 5 项 → 4 闭环 + 1 推后 (D1 50m-scale 待 30M 触发))** |
| 2026-06-23 | **Sprint 100 收口 (L4.20 test 1 CI fresh checkout 反噬治根 — 移除 `git cat-file -e` 历史对象验证，保留 HANDOFF commit SHA 字符串验证，模拟 CI shallow clone 4/4 PASS，v0.4.14.157 不变，main HEAD `0488cb0`，pytest 819/23/0，累计 50 sprint 0 debt，L4.x 20 条 stable)** |
| 2026-06-23 | **Sprint 101 收口 (全部收尾留尾治理 sprint — SSOT drift lint 2 records PASS + 跨文档 8/8 一致性 + L4.21 反 sprint 自我反馈闭环永久规则，0 业务代码、0 新留尾、VERSION 0.4.14.157 不 bump，pytest 819/23/0，累计 51 sprint 0 debt，L4.x 20 → 21 stable；Sprint 60+ 留尾 4 项闭环 + D1 按 30M 触发推后)** |
| 2026-06-24 | **Sprint 104 收口 (删 /visitor 路由别名 — 留尾治理 sprint 真业务必修 + /investigate + workflow 3 视角审查 3/3 agree (8.67/10) + /review PASS + /qa PASS + 12 步流程 + 3 文件 -25 行纯删除 + 推翻 Sprint 52 commit 50eb241 拍板 + 后端 /api/v1/visitor/* 100% 保留 (AudienceView 末尾访客段仍调 fetchVisitor*, #12 误判撤掉) + 2 次 amend 跟 Sprint 100+101+102+103 模式一致, v0.4.14.157 不变, main HEAD `03c9c08`, pytest 819/23/0, 累计 54 sprint 0 debt, L4.x 21 → 22 stable 新增 L4.22 (前端 sprint 收口必 rebuild dist + restart vite preview + activate core.hooksPath, Step 12.5/12.6 实战补 amend 闭环))** |
| 2026-06-30 | **Sprint 169 收口** (02 板块回购周期分布率最终收口 + worktree uvicorn 启动脚本 + W3 DQ/test fix + 复购周期 endpoint bug fix '全店' sentinel 守卫, v0.4.14.22 跨 67 sprint 不 bump stable, 累计 96→98 0 debt sprint 持续) |
| 2026-06-30 | **Sprint 170 收口** (RFM 8 象限 → R 区间 6 桶业务口径变更, `backend/semantic/segments.py:R_SEGMENT_ORDER` 公共 SSOT 沉淀, 11 service 同步 + e2e/spec drift 治根 18 sprint 滞后 stable 模式 (L4.23 永久规则) + 检测脚本自动化, v0.4.14.22) |
| 2026-06-30 | **Sprint 171 收口** (`/ad-hoc-query` 升级到 v2.0 两年对比分析模式 — 7 新子命令 two-year-overview / new-old-customer / rfm-repurchase / top-n / export-excel / dq-report / ask + XLSX 视觉 SSOT (深蓝 #1F4E79 / A 股红绿 #D32F2F #2E7D32 / 0 公式) + 防串台硬规则 (字段前缀 new_*/old_*/r_seg_*/channel_* 分离 + 每 sheet 独立 service 不复用中间 dict) + 18 pytest case 全部 PASS mock backend service + R 6 桶真实 SSOT 复用. 三端兼容: Claude Code + CodeBuddy CLI + WorkBuddy skill. 跨 sprint 真业务触发真因真发现模式: Codex 协作 handoff 走 AGENTS.md 自动注入, 架构师 Stage 1 / 实施者 Stage 2 / 审查 Stage 3 / 收口 Stage 4. 累计 100 sprint 0 debt 持续, VERSION 0.4.14.22 不变 (CLI 工具升级不影响 VERSION), pytest 813/72/0 (795 + 18 Sprint 171 case). L4.x 23 stable + 2 候选 (codegraph 实证 + 防串台字段前缀分离)) |
| 2026-06-30 | **Sprint 171 CI 治本** (3 commit: 1d7d22c lint F401 unused import + 33d4328 简化 sampling.spec.ts (跨 Sprint 169-170 reflow 后 .n-select scrollIntoView 不可靠, 跳过 Sprint 140 level switch 块 advisory) + ce5aef6 SamplingView.vue:97 data.buckets null guard (schema_test 模式 backend 不返回 buckets → TypeError: e.buckets is not iterable 污染 console). GitHub Actions UI 全部失败 + cancelled run 累计 354 个批量删除, 保留 success run 做 audit trail. main HEAD `ce5aef6` + origin/main 0 drift + CI 4/4 jobs 全绿. 实战 fix 模式 #52: CI 红 ≠ selector 问题, timeout 经常掩盖 console error 真因, 修 selector 后必看 consoleErrors 确认 UI 真实 bug) |
