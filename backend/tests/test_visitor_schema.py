...
def test_visitor_summary_member_join_rate_yoy_uses_pp_field():
    ...
    assert _has_constraint(f, -100.0, 100.0), (
        f"member_join_rate_yoy 必须是 PpField (ge=-100 le=100), 实际 metadata={f.metadata}. "
        f"修法: d660352 已是 PpField, 验 schema 没退化."
    )
...
def test_visitor_summary_member_join_rate_mom_uses_pp_field():
    ...
    assert _has_constraint(f, -100.0, 100.0), (
        f"member_join_rate_mom 必须是 PpField, 实际 metadata={f.metadata}"
    )
