    ...
    try:
        ...
        for i in range(50):
            conn.execute(
                ...
                [f"TAOKE_{i:03d}", "淘客", True, "2026-06-01 10:00:00", "Product T1"],
            )
        # 30 其他 (非淘客订单, 不动)
        for i in range(30):
            conn.execute(
...
class TestTaokeChannelRace:
    ...
    def test_drop_recreate_index_no_crash(self, temp_orders_with_channel_index):
        ...
        治根前 (无本次 fix): race 在 `SELECT COUNT(*) FROM orders WHERE channel = '淘客'`
        ...
        conn.execute("UPDATE orders SET channel = '其他' WHERE channel = '淘客'")
        after_count = conn.execute("SELECT COUNT(*) FROM orders WHERE channel = '淘客'").fetchone()[0]
        ...
        # 验证: 50 个原淘客订单全 reset 为'其他'
        assert after_count == 0, f"reset 后淘客应为 0, 实际 {after_count}"
    ...
    def test_idempotent_rerun(self, temp_orders_with_channel_index):
        ...
        for run in range(3):
            ...
            conn.execute("CREATE INDEX idx_orders_channel_member ON orders(channel, is_member)")
            conn.execute("COMMIT")
        # 3 次跑完, 所有 50 个原淘客订单都应 reset 为'其他' (幂等)
        n_other = conn.execute("SELECT COUNT(*) FROM orders WHERE channel = '其他'").fetchone()[0]
    ...
    def test_rollback_preserves_state(self, temp_orders_with_channel_index):
        ...
        # 验证: 50 淘客订单数未变
        n_taoke = conn.execute("SELECT COUNT(*) FROM orders WHERE channel = '淘客'").fetchone()[0]
        assert n_taoke == 50, f"ROLLBACK 应保留 50 淘客, 实际 {n_taoke}"
    ...
    def test_p6_2_keyword_re_mark_after_drop(self, temp_orders_with_channel_index):
        ...
        conn.execute("DROP INDEX IF EXISTS idx_orders_channel_pay_time")
        conn.execute("DROP INDEX IF EXISTS idx_orders_channel_member")
        # P6-2 关键词匹配: 20 个 NEW_OTHER (product_title 含 T1) 标淘客
        conn.execute("""
            UPDATE orders
            SET channel = '淘客'
            WHERE channel = '其他'
        ...
        # 验证: 50 原淘客 + 20 NEW_OTHER 标了淘客 = 70 (P6-2 关键词匹配, 没改原 50 淘客)
        assert n_taoke == 70, f"P6-2 后淘客应为 70 (50 原 + 20 NEW_OTHER), 实际 {n_taoke}"
        # 验证: 20 NEW_OTHER 是新标淘客 (跟 P6 订单号匹配不一样的逻辑, 通过 product_title LIKE 触发)
        n_new_other_taoke = conn.execute(
            "SELECT COUNT(*) FROM orders WHERE order_id LIKE 'NEW_OTHER_%' AND channel = '淘客'"
        ).fetchone()[0]
        assert n_new_other_taoke == 20, f"NEW_OTHER 标淘客应 20, 实际 {n_new_other_taoke}"
