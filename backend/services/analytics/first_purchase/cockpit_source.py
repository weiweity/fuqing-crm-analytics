"""Resolve a saved first-purchase SNAPSHOT analysis for cockpit HTTP.

Callers never pass client facts. Missing sources degrade that card only.
"""
...
from backend.services.analytics.first_purchase.asset_state import OPAQUE_ID
from backend.services.analytics.first_purchase.cockpit import (
    PLUGIN_TABLE,
    FirstPurchaseCockpitStore,
    spec_hash,
)
...
def _source_document(
    analysis_store: FirstPurchaseSavedAnalysisStore, principal: AnalyticsPrincipal,
    analysis_id: str, version: int,
) -> dict:
    try:
...
def resolve_trusted_analysis_card(
    ...
    try:
        snapshot = json.loads(canonical_json(
            FirstPurchaseSavedSnapshot.model_validate(document.get("snapshot")).model_dump(mode="json")
        ))
        facts = json.loads(canonical_json(
            FirstPurchaseFacts.model_validate(document.get("facts")).model_dump(mode="json")
        ))
        FirstPurchaseQueryRequest.model_validate(document.get("filters"))
    except _SOURCE_DECODE_ERRORS as error:
        raise AnalyticsError(422, "UNPROCESSABLE", "保存分析缺少合法冻结 SNAPSHOT。") from error
    if snapshot.get("run_id") != document.get("created_from_run_id"):
        raise AnalyticsError(409, "BINDING_CORRUPT", "分析快照与冻结来源不一致。")
    if facts.get("observation_days") != snapshot["resolved_filters"]["observation_days"]:
        raise AnalyticsError(409, "BINDING_CORRUPT", "分析 facts 与冻结条件不一致。")
    if document.get("filter_hash") != snapshot["resolved_filters"]["filter_hash"]:
...
def _safe_ref(card: dict) -> dict | None:
    ...
    try:
        return FirstPurchaseCockpitAnalysisRef.model_validate(raw).model_dump(mode="json")
...
def _safe_layout(card: dict) -> dict:
    try:
        return FirstPurchaseCockpitLayout.model_validate(card.get("layout")).model_dump(mode="json")
    except ValidationError:
...
def _safe_plugin(card: dict) -> dict | None:
    try:
        return FirstPurchaseCockpitTablePlugin.model_validate(card.get("plugin_ref")).model_dump(mode="json")
    except ValidationError:
        return None
...
def _error_card(card: object, error: AnalyticsError) -> dict:
    ...
    return FirstPurchaseCockpitCardUnavailable.model_validate(projected).model_dump(mode="json")
...
def _bind_cached_card(card: dict, trusted: dict) -> None:
    ...
    try:
        cached_snapshot = json.loads(canonical_json(
            FirstPurchaseSavedSnapshot.model_validate(card.get("snapshot")).model_dump(mode="json")
        ))
        cached_facts = json.loads(canonical_json(
            FirstPurchaseFacts.model_validate(card.get("facts")).model_dump(mode="json")
        ))
...
def authorize_existing_card(
    analysis_store: FirstPurchaseSavedAnalysisStore, principal: AnalyticsPrincipal, card: dict,
) -> dict:
    ref = _safe_ref(card)
    if ref is None:
...
def authorize_http_op(
    analysis_store: FirstPurchaseSavedAnalysisStore,
    cockpit_store: FirstPurchaseCockpitStore,
    principal: AnalyticsPrincipal,
    dashboard_id: str,
    payload: FirstPurchaseCockpitOp,
    if_match: str,
) -> dict:
    if payload.op == "add":
...
def _ok_card(card: dict, trusted: dict) -> dict:
    ...
    projected = {
        ...
        "limitations": list(trusted["limitations"]),
        "effective_spec_hash": spec_hash(trusted["analysis_ref"], trusted["snapshot"], local_filters),
        "freshness": "PINNED",
        "source_status": "OK",
    }
    return FirstPurchaseCockpitCardOk.model_validate(projected).model_dump(mode="json")


def project_card(
    analysis_store: FirstPurchaseSavedAnalysisStore, principal: AnalyticsPrincipal, card: object,
) -> dict:
    payload = card if isinstance(card, dict) else {}
    try:
        ...
        try:
            FirstPurchaseCockpitLayout.model_validate(payload.get("layout"))
        except ValidationError:
...
def project_dashboard(
    analysis_store: FirstPurchaseSavedAnalysisStore, principal: AnalyticsPrincipal, payload: dict, *,
    base_version: int,
) -> dict:
