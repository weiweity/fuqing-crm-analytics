...
class TierFlowRow(BaseModel):
    ...
    repurchase_rate_current: "RatioField" = Field(default=0.0, description="回购率 0-1 decimal")
    ...
    repurchase_rate_comp: "RatioField" = Field(default=0.0, description="回购率 0-1 decimal")
    repurchase_gsv_comp: float = Field(default=0.0)
    repurchase_gsv_ratio_comp: "RatioField" = Field(default=0.0, description="回购GSV占比 0-1 decimal")
    hist_users_prev2: int = Field(default=0)
    repurchase_users_prev2: int = Field(default=0)
    repurchase_rate_prev2: "RatioField" = Field(default=0.0, description="回购率 0-1 decimal")
    repurchase_gsv_prev2: float = Field(default=0.0)
    repurchase_gsv_ratio_prev2: "RatioField" = Field(default=0.0, description="回购GSV占比 0-1 decimal")
    yoy_hist_users: Optional[float] = Field(None)
    yoy_repurchase_users: Optional[float] = Field(None)
    # Sprint 31.2: 业务实证 (semantic.calculations.py:70-80) yoy_repurchase_rate 函数 = yoy_ratio
    # = (cur - comp) * 100, 返回 pp 差. 跟 yoy_repurchase_gsv_ratio_ppt 是同一函数
    yoy_repurchase_rate: Optional["PpField"] = Field(None, description="回购率同比 (pp 差)")
...
class NewCustomerConversionFunnel(BaseModel):
    ...
    day7_rate: "RatioField" = Field(default=0.0, description="7日回购率 0-1 decimal")
    day30_repurchase: int = Field(default=0)
    day30_rate: "RatioField" = Field(default=0.0, description="30日回购率 0-1 decimal")
    day90_repurchase: int = Field(default=0)
    day90_rate: "RatioField" = Field(default=0.0, description="90日回购率 0-1 decimal")
    year_repurchase: int = Field(default=0)
    year_rate: "RatioField" = Field(default=0.0, description="年度回购率 0-1 decimal")
